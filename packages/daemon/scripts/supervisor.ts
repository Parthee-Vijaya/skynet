...
function parseLifecycleMessage(msg: unknown): WorkerLifecycleMessage | null {
  ...
  if (type === "skynet:shutdown") {
    return { type: "skynet:shutdown" };
  }
  if (type === "skynet:ready") {
    const listen = (msg as { listen?: unknown }).listen;
    if (typeof listen !== "string" || listen.trim().length === 0) {
      return null;
    }
    return { type: "skynet:ready", listen };
  }
  if (type === "skynet:restart") {
...
export function runSupervisor(options: SupervisorOptions): void {
  ...
  const spawnWorker = () => {
    ...
    child.on("message", (msg: unknown) => {
      ...
      if (lifecycleMessage.type === "skynet:ready") {
      ...
      if (lifecycleMessage.type === "skynet:shutdown") {
