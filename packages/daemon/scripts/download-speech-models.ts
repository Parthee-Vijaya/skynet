...
function parseArgs(argv: string[]): { modelsDir: string; modelIds: LocalSpeechModelId[] } {
  const home = resolveSkynetHome();
  let modelsDir = process.env.SKYNET_LOCAL_MODELS_DIR || `${home}/models/local-speech`;
  ...
  if (modelIds.length === 0) {
    const stt = (process.env.SKYNET_LOCAL_STT_MODEL ||
      DEFAULT_LOCAL_STT_MODEL) as LocalSpeechModelId;
    const tts = (process.env.SKYNET_LOCAL_TTS_MODEL ||
