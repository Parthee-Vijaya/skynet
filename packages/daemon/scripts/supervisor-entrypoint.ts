...
import { resolveSkynetHome } from "../src/server/skynet-home.js";
...
function resolvePackagedNodeEntrypointRunnerPath(currentScriptPath: string): string | null {
  const packageMarker = `${path.sep}node_modules${path.sep}@getskynet${path.sep}server${path.sep}`;
...
async function main(): Promise<void> {
  ...
  const workerEnv: NodeJS.ProcessEnv = { ...process.env, SKYNET_SUPERVISED: "1" };
  ...
  const skynetHome = resolveSkynetHome(workerEnv);
  ...
  runSupervisor({
    ...
    onWorkerReady: async ({ listen }) => {
      await updatePidLock(skynetHome, { listen }, { ownerPid: process.pid });
    },
