...
function usage(): string {
  return [
    ...
    "",
    "Env fallbacks:",
    "  SKYNET_LOCAL_MODELS_DIR, SKYNET_LOCAL_STT_MODEL",
  ].join("\n");
}
...
function parseArgs(argv: string[]): CliOptions {
  ...
  const defaultModelsDir =
    process.env.SKYNET_LOCAL_MODELS_DIR ?? path.join(skynetHome, "models", "local-speech");

  const positional: string[] = [];
  let outPath: string | undefined;
  let model = LocalSttModelIdSchema.parse(
    process.env.SKYNET_LOCAL_STT_MODEL ?? DEFAULT_LOCAL_STT_MODEL,
  );
