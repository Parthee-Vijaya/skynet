...
describe("DaemonClient", () => {
    ...
    mock.triggerMessage(
      wrapSessionMessage({
        ...
        payload: {
          ...
          detail: {
            type: "worktree_setup",
            worktreePath: "/tmp/project/.skynet/worktrees/feature-a",
            ...
            commands: [
              {
                index: 1,
                command: "npm install",
                cwd: "/tmp/project/.skynet/worktrees/feature-a",
                log: "phase-one\n",
                status: "running",
                exitCode: null,
              },
  ...
  test("sends structured attachments with create_skynet_worktree_request", async () => {
    const logger = createMockLogger();
    const mock = createMockTransport();
    ...
    const createPromise = client.createSkynetWorktree({
      cwd: "/tmp/project",
      worktreeSlug: "review-pr-123",
      attachments: [
        {
  ...
  test("sends worktree base-ref fields in create_skynet_worktree_request", async () => {
    const logger = createMockLogger();
    const mock = createMockTransport();
  ...
  test("omitting create_skynet_worktree_request worktree base-ref fields preserves legacy wire shape", async () => {
    const logger = createMockLogger();
    const mock = createMockTransport();
    ...
    mock.triggerMessage(
      JSON.stringify({
        ...
        message: {
          ...
          payload: {
            directories: ["/Users/test/projects/skynet"],
