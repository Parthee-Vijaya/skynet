...
import type {
  ...
  SkynetWorktreeListResponse,
  SkynetWorktreeArchiveResponse,
  ProjectIconResponse,
...
export interface CreateSkynetWorktreeInput
  extends Pick<
    CreateSkynetWorktreeRequest,
...
type SkynetWorktreeListPayload = SkynetWorktreeListResponse["payload"];
type SkynetWorktreeArchivePayload = SkynetWorktreeArchiveResponse["payload"];
type CreateSkynetWorktreePayload = Extract<
  SessionOutboundMessage,
...
export class DaemonClient {
  ...
  async getSkynetWorktreeList(
    input: { cwd?: string; repoRoot?: string },
    requestId?: string,
  ): Promise<SkynetWorktreeListPayload> {
    return this.sendCorrelatedSessionRequest({
      requestId,
      message: {
      ...
      responseType: "skynet_worktree_list_response",
      timeout: 60000,
    });
  }

  async archiveSkynetWorktree(
    input: { worktreePath?: string; repoRoot?: string; branchName?: string },
    requestId?: string,
  ): Promise<SkynetWorktreeArchivePayload> {
    return this.sendCorrelatedSessionRequest({
      requestId,
      message: {
      ...
      responseType: "skynet_worktree_archive_response",
      timeout: 20000,
    });
  }

  async createSkynetWorktree(
    input: CreateSkynetWorktreeInput,
    requestId?: string,
  ): Promise<CreateSkynetWorktreePayload> {
    return this.sendCorrelatedSessionRequest({
      requestId,
      message: {
      ...
      responseType: "create_skynet_worktree_response",
      timeout: 60000,
    });
  }
