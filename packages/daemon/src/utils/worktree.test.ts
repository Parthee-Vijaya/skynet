...
import {
  ...
  deleteSkynetWorktree,
  getScriptConfigs,
  getWorktreeTerminalSpecs,
  isServiceScript,
  isSkynetOwnedWorktreeCwd,
  listSkynetWorktrees,
...
import { getSkynetWorktreeMetadataPath } from "./worktree-metadata.js";
...
describe.skipIf(process.platform === "win32")("createWorktree", () => {
  ...
  it("creates a worktree for the current branch (main)", async () => {
    ...
    expect(result.worktreePath).toBe(join(skynetHome, "worktrees", projectHash, "hello-world"));
    expect(existsSync(result.worktreePath)).toBe(true);
    expect(existsSync(join(result.worktreePath, "file.txt"))).toBe(true);
    const metadataPath = getSkynetWorktreeMetadataPath(result.worktreePath);
    expect(existsSync(metadataPath)).toBe(true);
    const metadata = JSON.parse(readFileSync(metadataPath, "utf8"));
    expect(metadata).toMatchObject({ version: 1, baseRefName: "main" });
  });

  it.skip("detects skynet-owned worktrees across realpath differences (macOS /var vs /private/var)", async () => {
    ...
    const varSkynetHome = join(varTempDir, "skynet-home");
    ...
    const ownership = await isSkynetOwnedWorktreeCwd(privateWorktreePath, {
      skynetHome: varSkynetHome,
    });
  ...
  it("reports repoRoot as the repository root for skynet-owned worktrees", async () => {
    const result = await createLegacyWorktreeForTest({
      branchName: "main",
      cwd: repoDir,
      baseBranch: "main",
    ...
    const ownership = await isSkynetOwnedWorktreeCwd(result.worktreePath, { skynetHome });
    expect(ownership.allowed).toBe(true);
  ...
  it("treats non-git directories as non-worktrees without throwing", async () => {
    ...
    const ownership = await isSkynetOwnedWorktreeCwd(nonGitDir, { skynetHome });
  ...
  it("creates a worktree with a new branch", async () => {
    ...
    expect(result.worktreePath).toBe(join(skynetHome, "worktrees", projectHash, "my-feature"));
    expect(existsSync(result.worktreePath)).toBe(true);
  ...
  it("fetches a GitHub PR branch, checks it out, writes metadata, and runs setup", async () => {
    ...
    writeFileSync(
      join(remoteCloneDir, "skynet.json"),
  ...
  it("handles branch name collision by adding suffix", async () => {
    ...
    expect(result.worktreePath).toBe(join(skynetHome, "worktrees", projectHash, "hello"));
    expect(existsSync(result.worktreePath)).toBe(true);
  ...
  it("runs setup commands from skynet.json", async () => {
    // Create skynet.json with setup commands
    const skynetConfig = {
      worktree: {
        setup: [
          'echo "source=$SKYNET_SOURCE_CHECKOUT_PATH" > setup.log',
          'echo "root_alias=$SKYNET_ROOT_PATH" >> setup.log',
          'echo "worktree=$SKYNET_WORKTREE_PATH" >> setup.log',
          'echo "branch=$SKYNET_BRANCH_NAME" >> setup.log',
          'echo "port=$SKYNET_WORKTREE_PORT" >> setup.log',
        ],
      },
    };
    writeFileSync(join(repoDir, "skynet.json"), JSON.stringify(skynetConfig));
    execSync('git add skynet.json && git -c commit.gpgsign=false commit -m "add skynet.json"', {
      cwd: repoDir,
    });
  ...
  it("streams setup command progress events while commands are executing", async () => {
    ...
    writeFileSync(join(repoDir, "skynet.json"), JSON.stringify(skynetConfig));
    execSync('git add skynet.json && git -c commit.gpgsign=false commit -m "add streaming setup"', {
      cwd: repoDir,
    });
  ...
  it("reuses persisted worktree runtime port across resolutions", async () => {
    ...
    expect(second.SKYNET_WORKTREE_PORT).toBe(first.SKYNET_WORKTREE_PORT);
  });
  ...
  it("fails runtime env resolution when persisted port is in use", async () => {
    ...
    const port = Number(env.SKYNET_WORKTREE_PORT);
  ...
  it("cleans up worktree if setup command fails", async () => {
    // Create skynet.json with failing setup command
    const skynetConfig = {
      worktree: {
        setup: ["exit 1"],
      },
    ...
    const expectedWorktreePath = join(skynetHome, "worktrees", "test-repo", "fail-test");
  ...
  it("reads worktree terminal specs from skynet.json with optional name", async () => {
    const skynetConfig = {
      worktree: {
        terminals: [
...
describe("skynet worktree manager", () => {
  let tempDir: string;
  let repoDir: string;
  let skynetHome: string;
  ...
  it("isolates worktree roots for repositories that share the same directory name", async () => {
    ...
    const repoAWorktrees = await listSkynetWorktrees({ cwd: repoA, skynetHome });
    const repoBWorktrees = await listSkynetWorktrees({ cwd: repoB, skynetHome });
  ...
  it("lists and deletes skynet worktrees under ~/.skynet/worktrees/{hash}", async () => {
    ...
    const worktrees = await listSkynetWorktrees({ cwd: repoDir, skynetHome });
    const paths = worktrees.map((worktree) => worktree.path).sort();
    expect(paths).toEqual([first.worktreePath, second.worktreePath].sort());

    await deleteSkynetWorktree({ cwd: repoDir, worktreePath: first.worktreePath, skynetHome });
    expect(existsSync(first.worktreePath)).toBe(false);

    const remaining = await listSkynetWorktrees({ cwd: repoDir, skynetHome });
    expect(remaining.map((worktree) => worktree.path)).toEqual([second.worktreePath]);
  });

  it("deletes a skynet worktree even when given a subdirectory path", async () => {
    ...
    await deleteSkynetWorktree({ cwd: repoDir, worktreePath: nestedDir, skynetHome });
  ...
  it("runs teardown commands from skynet.json before deleting a worktree", async () => {
    const skynetConfig = {
      worktree: {
        teardown: [
          'echo "source=$SKYNET_SOURCE_CHECKOUT_PATH" > "$SKYNET_SOURCE_CHECKOUT_PATH/teardown.log"',
          'echo "root_alias=$SKYNET_ROOT_PATH" >> "$SKYNET_SOURCE_CHECKOUT_PATH/teardown.log"',
          'echo "worktree=$SKYNET_WORKTREE_PATH" >> "$SKYNET_SOURCE_CHECKOUT_PATH/teardown.log"',
          'echo "branch=$SKYNET_BRANCH_NAME" >> "$SKYNET_SOURCE_CHECKOUT_PATH/teardown.log"',
          'echo "port=$SKYNET_WORKTREE_PORT" >> "$SKYNET_SOURCE_CHECKOUT_PATH/teardown.log"',
        ],
      },
    };
    writeFileSync(join(repoDir, "skynet.json"), JSON.stringify(skynetConfig));
    execSync(
      'git add skynet.json && git -c commit.gpgsign=false commit -m "add teardown commands"',
      {
        cwd: repoDir,
      },
    );
    ...
    await deleteSkynetWorktree({ cwd: repoDir, worktreePath: created.worktreePath, skynetHome });
    expect(existsSync(created.worktreePath)).toBe(false);
    ...
    expect(teardownLog).toContain(`port=${runtimeEnv.SKYNET_WORKTREE_PORT}`);
  });

  it("omits SKYNET_WORKTREE_PORT from teardown env when runtime metadata is missing", async () => {
    const skynetConfig = {
      worktree: {
        teardown: [
          'echo "port=${SKYNET_WORKTREE_PORT-unset}" > "$SKYNET_SOURCE_CHECKOUT_PATH/teardown-port.log"',
        ],
      },
    };
    writeFileSync(join(repoDir, "skynet.json"), JSON.stringify(skynetConfig));
    execSync(
      'git add skynet.json && git -c commit.gpgsign=false commit -m "add teardown port logging"',
      { cwd: repoDir },
    );
  ...
  it("does not remove worktree when a teardown command fails", async () => {
    const skynetConfig = {
      worktree: {
        teardown: [
          'echo "started" > "$SKYNET_SOURCE_CHECKOUT_PATH/teardown-start.log"',
    ...
    writeFileSync(join(repoDir, "skynet.json"), JSON.stringify(skynetConfig));
    execSync(
      'git add skynet.json && git -c commit.gpgsign=false commit -m "add failing teardown commands"',
      { cwd: repoDir },
    );
    ...
    await expect(
      deleteSkynetWorktree({ cwd: repoDir, worktreePath: created.worktreePath, skynetHome }),
  ...
  it("treats a worktree as skynet-owned even when its .git admin is missing", async () => {
    const created = await createLegacyWorktreeForTest({
    ...
    const ownership = await isSkynetOwnedWorktreeCwd(created.worktreePath, { skynetHome });
    expect(ownership.allowed).toBe(true);
  });

  it("rejects paths that are not under the skynet worktrees root", async () => {
    const outsidePath = join(tempDir, "outside-skynet-home");
    mkdirSync(outsidePath, { recursive: true });

    const ownership = await isSkynetOwnedWorktreeCwd(outsidePath, { skynetHome });
  ...
  it("rejects the worktrees root itself and the per-repo hash dir", async () => {
    const projectHash = await deriveWorktreeProjectHash(repoDir);
    const worktreesRoot = join(skynetHome, "worktrees");
    const projectHashDir = join(worktreesRoot, projectHash);
    mkdirSync(projectHashDir, { recursive: true });

    await expect(isSkynetOwnedWorktreeCwd(worktreesRoot, { skynetHome })).resolves.toMatchObject({
      allowed: false,
    });
    await expect(isSkynetOwnedWorktreeCwd(projectHashDir, { skynetHome })).resolves.toMatchObject({
      allowed: false,
    });
  });
