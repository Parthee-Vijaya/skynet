...
import {
  ...
  readSkynetWorktreeMetadata,
  readSkynetWorktreeRuntimePort,
  writeSkynetWorktreeMetadata,
  writeSkynetWorktreeRuntimeMetadata,
} from "./worktree-metadata.js";
import { runGitCommand } from "./run-git-command.js";
import { resolveSkynetHome } from "../server/skynet-home.js";
...
export type WorktreeRuntimeEnv = {
  SKYNET_SOURCE_CHECKOUT_PATH: string;
  SKYNET_ROOT_PATH: string;
  SKYNET_WORKTREE_PATH: string;
  SKYNET_BRANCH_NAME: string;
  SKYNET_WORKTREE_PORT: string;
};
...
export interface SkynetWorktreeInfo {
  path: string;
  createdAt: string;
  branchName?: string;
  head?: string;
...
export type SkynetWorktreeOwnership = {
  allowed: boolean;
  repoRoot?: string;
  worktreeRoot?: string;
  worktreePath?: string;
...
function readSkynetConfig(repoRoot: string): SkynetConfig | null {
  const skynetConfigPath = join(repoRoot, "skynet.json");
  if (!existsSync(skynetConfigPath)) {
    return null;
  }
  try {
    return JSON.parse(readFileSync(skynetConfigPath, "utf8"));
  } catch {
    throw new Error(`Failed to parse skynet.json`);
  }
}

export function getWorktreeSetupCommands(repoRoot: string): string[] {
  const config = readSkynetConfig(repoRoot);
...
export async function runWorktreeSetupCommands(options: {
  ...
  // Read skynet.json from the worktree (it will have the same content as the source repo)
...
export async function resolveWorktreeRuntimeEnv(options: {
  ...
  let worktreePort = readSkynetWorktreeRuntimePort(options.worktreePath);
  if (worktreePort === null) {
    worktreePort = await getAvailablePort();
    const metadata = readSkynetWorktreeMetadata(options.worktreePath);
    if (metadata) {
      writeSkynetWorktreeRuntimeMetadata(options.worktreePath, { worktreePort });
    }
  } else {
  ...
  return {
    ...
    // source checkout.
    SKYNET_SOURCE_CHECKOUT_PATH: repoRootPath,
    // Backward-compatible alias.
    SKYNET_ROOT_PATH: repoRootPath,
    SKYNET_WORKTREE_PATH: options.worktreePath,
    SKYNET_BRANCH_NAME: branchName,
    SKYNET_WORKTREE_PORT: String(worktreePort),
  };
}
...
export async function runWorktreeTeardownCommands(options: {
  ...
  const branchName =
    options.branchName ?? (await resolveBranchNameForWorktreePath(options.worktreePath));
  const worktreePort = readSkynetWorktreeRuntimePort(options.worktreePath);
  ...
  const teardownEnv: NodeJS.ProcessEnv = {
    ...
    SKYNET_WORKTREE_PATH: options.worktreePath,
    SKYNET_BRANCH_NAME: branchName,
    ...(worktreePort !== null ? { SKYNET_WORKTREE_PORT: String(worktreePort) } : {}),
  };
...
export async function getSkynetWorktreesRoot(cwd: string, skynetHome?: string): Promise<string> {
  const home = skynetHome ? resolve(skynetHome) : resolveSkynetHome();
...
export async function computeWorktreePath(
  ...
  const worktreesRoot = await getSkynetWorktreesRoot(cwd, skynetHome);
...
export async function isSkynetOwnedWorktreeCwd(
  cwd: string,
  options?: { skynetHome?: string },
): Promise<SkynetWorktreeOwnership> {
  ...
  const skynetHome = options?.skynetHome ? resolve(options.skynetHome) : resolveSkynetHome();
  const skynetWorktreesPrefix = normalizePathForOwnership(join(skynetHome, "worktrees")) + sep;

  // Ownership is defined by the path living under $SKYNET_HOME/worktrees/<hash>/<slug>[/...].
  // The <hash>/<slug> prefix is Skynet-private — nothing else writes there — so the
  // path shape alone is sufficient proof of ownership, even when git has already
  // forgotten about the worktree.
  if (!resolvedCwd.startsWith(skynetWorktreesPrefix)) {
    return {
      allowed: false,
  ...
  const relative = resolvedCwd.slice(skynetWorktreesPrefix.length);
  ...
  const worktreesRoot = join(skynetHome, "worktrees", parts[0]!);
  return {
    allowed: true,
    ...(repoRoot !== undefined ? { repoRoot } : {}),
...
type ParsedSkynetWorktreeInfo = Omit<SkynetWorktreeInfo, "createdAt">;

function parseWorktreeList(output: string): ParsedSkynetWorktreeInfo[] {
  const entries: ParsedSkynetWorktreeInfo[] = [];
  let current: ParsedSkynetWorktreeInfo | null = null;
...
export async function listSkynetWorktrees({
  cwd,
  skynetHome,
}: {
  cwd: string;
  skynetHome?: string;
}): Promise<SkynetWorktreeInfo[]> {
  const worktreesRoot = await getSkynetWorktreesRoot(cwd, skynetHome);
...
export async function resolveExistingWorktreeForSlug({
  ...
  const worktrees = await listSkynetWorktrees({
    cwd: repoRoot,
    skynetHome,
  });
...
export async function resolveSkynetWorktreeRootForCwd(
  cwd: string,
  options?: { skynetHome?: string },
  ...
  const worktreesRoot = await getSkynetWorktreesRoot(cwd, options?.skynetHome);
  ...
  const knownWorktrees = await listSkynetWorktrees({
    cwd,
...
export async function deleteSkynetWorktree({
  cwd,
  worktreePath,
  worktreeSlug,
  worktreesRoot,
  ...
  if (worktreesRoot) {
    ...
    resolvedWorktreesRoot = await getSkynetWorktreesRoot(cwd, skynetHome);
  } else {
    throw new Error("cwd or worktreesRoot is required to delete a Skynet worktree");
  }
  ...
  const resolvedWorktree =
    (await resolveSkynetWorktreeRootForCwd(requestedPath, { skynetHome }))?.worktreePath ??
    resolvedRequested;

  if (!resolvedWorktree.startsWith(resolvedRoot)) {
    throw new Error("Refusing to delete non-Skynet worktree");
  }
...
export const createWorktree = async ({
  ...
  let worktreePath = join(await getSkynetWorktreesRoot(cwd, skynetHome), worktreeSlug);
  ...
  writeSkynetWorktreeMetadata(worktreePath, { baseRefName: sourcePlan.metadataBaseRefName });
    ...
    case "checkout-github-pr": {
      ...
      return {
        ...
        ...(source.pushRemoteUrl
          ? {
              pushRemote: {
                name: `skynet-pr-${source.githubPrNumber}`,
...
function normalizeRequiredBaseBranch(baseBranch: string): string {
  ...
  if (!normalizedBaseBranch) {
    throw new Error("Base branch is required when creating a Skynet worktree");
  }
  if (normalizedBaseBranch === "HEAD") {
    throw new Error("Base branch cannot be HEAD when creating a Skynet worktree");
  }
