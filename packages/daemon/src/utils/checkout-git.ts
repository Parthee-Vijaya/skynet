...
import { isSkynetOwnedWorktreeCwd } from "./worktree.js";
import { requireSkynetWorktreeBaseRefName } from "./worktree-metadata.js";
...
export type CheckoutStatusGitNonSkynet = {
  isGit: true;
  repoRoot: string;
...
export type CheckoutStatusGitSkynet = {
  isGit: true;
  repoRoot: string;
  mainRepoRoot: string;
  currentBranch: string | null;
...
export type CheckoutStatusGit = CheckoutStatusGitNonSkynet | CheckoutStatusGitSkynet;
...
export async function getMainRepoRoot(cwd: string): Promise<string> {
  ...
  const nonBareNonSkynet = worktrees.filter((wt) => !wt.isBare && !isSkynetWorktreePath(wt.path));
  const childrenOfBareRepo = nonBareNonSkynet.filter((wt) => isDescendantPath(wt.path, normalized));
  const mainChild = childrenOfBareRepo.find((wt) => basename(wt.path) === "main");
  return mainChild?.path ?? childrenOfBareRepo[0]?.path ?? nonBareNonSkynet[0]?.path ?? normalized;
}
...
/** Check whether a path contains a `.skynet/worktrees/` segment (both `/` and `\`). */
export function isSkynetWorktreePath(p: string): boolean {
  return /[/\\]\.skynet[/\\]worktrees[/\\]/.test(p);
}
...
type ConfiguredBaseRefForCwd =
  | { baseRef: null; isSkynetOwnedWorktree: false }
  | { baseRef: string; isSkynetOwnedWorktree: true };
...
async function getConfiguredBaseRefForCwd(
  ...
  if (!/[\\/]worktrees[\\/]/.test(cwd)) {
    return { baseRef: null, isSkynetOwnedWorktree: false };
  }

  const ownership = await isSkynetOwnedWorktreeCwd(cwd, { skynetHome: context?.skynetHome });
  ...
  return {
    baseRef: requireSkynetWorktreeBaseRefName(worktreeRoot),
    isSkynetOwnedWorktree: true,
  };
}
...
export async function getCheckoutStatus(
  ...
  if (configured.isSkynetOwnedWorktree) {
...
export async function getCheckoutDiff(
  ...
  if (compare.mode === "uncommitted") {
    ...
    if (configured.isSkynetOwnedWorktree && compare.baseRef && compare.baseRef !== baseRef) {
...
export async function mergeToBase(
  ...
  if (configured.isSkynetOwnedWorktree && options.baseRef && options.baseRef !== baseRef) {
...
export async function createPullRequest(
  ...
  if (configured.isSkynetOwnedWorktree && options.base && options.base !== base) {
