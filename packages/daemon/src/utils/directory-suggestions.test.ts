...
describe("searchHomeDirectories", () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(path.join(homeDir, "projects", "skynet"), { recursive: true });
  ...
  it("returns only existing directories", async () => {
    ...
    expect(results).toContain(path.join(homeDir, "projects", "skynet"));
  ...
  it("supports home-relative path query syntax", async () => {
    ...
    expect(results).toEqual([path.join(homeDir, "projects", "skynet")]);
  });
...
describe("searchWorkspaceEntries", () => {
  ...
  beforeEach(() => {
    ...
    writeFileSync(path.join(workspaceDir, "README.md"), "# skynet\n");
    writeFileSync(
