...
function createGitHubServiceForStatus(
  ...
  return {
    ...
    getPullRequest: async () => ({
      number: 1,
      title: "PR",
      url: "https://github.com/getskynet/skynet/pull/1",
      state: "OPEN",
      body: null,
      baseRefName: "main",
      headRefName: "feature",
...
function createPullRequestStatus(overrides?: Partial<GitHubCurrentPullRequestStatus>) {
  return {
    url: "https://github.com/getskynet/skynet/pull/123",
    title: "Ship feature",
    state: "open",
    baseRefName: "main",
    headRefName: "feature",
...
`;
  ...
  it("returns checkout root metadata for normal repos", async () => {
    ...
    expect(status.isSkynetOwnedWorktree).toBe(false);
  ...
  it("handles status/diff/commit in a .skynet worktree", async () => {
    ...
    const status = await getCheckoutStatus(result.worktreePath, { skynetHome });
    expect(status.isGit).toBe(true);
    expect(status.repoRoot).toBe(result.worktreePath);
    expect(status.isDirty).toBe(true);
    expect(status.isSkynetOwnedWorktree).toBe(true);
    expect(status.mainRepoRoot).toBe(repoDir);

    const diff = await getCheckoutDiff(result.worktreePath, { mode: "uncommitted" }, { skynetHome });
    expect(diff.diff).toContain("-hello");
    expect(diff.diff).toContain("+worktree change");

    await commitAll(result.worktreePath, "worktree update");

    const cleanStatus = await getCheckoutStatus(result.worktreePath, { skynetHome });
    expect(cleanStatus.isDirty).toBe(false);
  ...
  it("returns checkout root metadata for .skynet worktrees", async () => {
    const result = await createLegacyWorktreeForTest({
      branchName: "main",
      cwd: repoDir,
      baseBranch: "main",
  ...
  it("returns mainRepoRoot pointing to first non-bare worktree for bare repos", async () => {
    ...
    const status = await getCheckoutStatus(worktree.worktreePath, { skynetHome });
    expect(status.isGit).toBe(true);
    expect(status.isSkynetOwnedWorktree).toBe(true);
  ...
  it("merges the current branch into base from a worktree checkout", async () => {
    ...
    await mergeToBase(worktree.worktreePath, { baseRef: "main" }, { skynetHome });
    ...
    const statusAfterMerge = await getCheckoutStatus(worktree.worktreePath, { skynetHome });
  ...
  it("reports the base worktree cwd when merge-to-base mutates a separate checkout", async () => {
    ...
    const mutatedCwd = await mergeToBase(featureWorktree.worktreePath, {}, { skynetHome });
  ...
  it("disables GitHub features when gh is unavailable", async () => {
    execSync("git remote add origin https://github.com/getskynet/skynet.git", { cwd: repoDir });
          ...
          checks: [
            {
              name: "server-tests",
              status: "success",
              url: "https://github.com/getskynet/skynet/actions/runs/123",
              workflow: "Server CI",
              duration: "2m 14s",
            },
          ],
  ...
  it("returns closed-unmerged PR status without marking it as merged", async () => {
    ...
    const status = await getPullRequestStatus(
      ...
      createGitHubServiceForStatus(
        createPullRequestStatus({
          url: "https://github.com/getskynet/skynet/pull/999",
    ...
    try {
      ...
      github.getCurrentPullRequestStatus = async () => {
        ...
        return createPullRequestStatus({
          url: `https://github.com/getskynet/skynet/pull/${callCount}`,
        });
      };
      const first = await getPullRequestStatus(repoDir, github);
      await sleep(80);
  ...
  it("uses stored baseRefName for Skynet worktrees (no heuristics)", async () => {
    ...
    const baseDiff = await getCheckoutDiff(worktree.worktreePath, { mode: "base" }, { skynetHome });
    expect(baseDiff.diff).toContain("feature.txt");
    expect(baseDiff.diff).not.toContain("file.txt");
  });

  it("excludes dirty working tree changes from Skynet worktree base diffs", async () => {
    const worktree = await createLegacyWorktreeForTest({
      branchName: "feature",
      cwd: repoDir,
      baseBranch: "main",
  ...
  it("merges to stored baseRefName when baseRef is not provided", async () => {
    ...
    // Create a Skynet worktree configured to use develop as base.
    const worktree = await createLegacyWorktreeForTest({
      branchName: "feature",
      cwd: repoDir,
      baseBranch: "develop",
    ...
    await mergeToBase(worktree.worktreePath, {}, { skynetHome });
  ...
  it("throws if Skynet worktree base metadata is missing", async () => {
    const worktree = await createLegacyWorktreeForTest({
      branchName: "main",
      cwd: repoDir,
      baseBranch: "main",
    ...
    const metadataPath = getSkynetWorktreeMetadataPath(worktree.worktreePath);
    rmSync(metadataPath, { force: true });

    await expect(getCheckoutStatus(worktree.worktreePath, { skynetHome })).rejects.toThrow(/base/i);
    await expect(
      getCheckoutDiff(worktree.worktreePath, { mode: "base" }, { skynetHome }),
    ).rejects.toThrow(/base/i);
    await expect(mergeToBase(worktree.worktreePath, {}, { skynetHome })).rejects.toThrow(/base/i);
  });
  ...
  describe("parseWorktreeList", () => {
    it("parses porcelain worktree output", () => {
      const output = [
        ...
        "",
        "worktree /home/user/.skynet/worktrees/feature",
      ...
      expect(entries[1]).toEqual({
        path: "/home/user/.skynet/worktrees/feature",
  ...
  describe("isSkynetWorktreePath", () => {
    it("matches Unix .skynet/worktrees/ paths", () => {
      expect(isSkynetWorktreePath("/home/user/.skynet/worktrees/feature")).toBe(true);
    });

    it("matches Windows .skynet\\worktrees\\ paths", () => {
      expect(isSkynetWorktreePath("C:\\Users\\dev\\.skynet\\worktrees\\feature")).toBe(true);
    });

    it("rejects paths without .skynet/worktrees segment", () => {
      expect(isSkynetWorktreePath("/home/user/repo")).toBe(false);
      expect(isSkynetWorktreePath("C:\\Users\\dev\\repo")).toBe(false);
    });
  });
