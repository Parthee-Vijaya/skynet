...
describe("Terminal", () => {
  ...
  describe("createTerminal", () => {
    ...
    it("sets zsh wrapper env when spawning zsh", () => {
      const resolvedEnv = buildTerminalEnvironment({
        shell: "/bin/zsh",
        env: {
          HOME: "/tmp/skynet-home",
          ZDOTDIR: "/tmp/skynet-zdotdir",
        },
      });

      expect(resolvedEnv.TERM).toBe("xterm-256color");
      expect(resolvedEnv.SKYNET_ZSH_ZDOTDIR).toBe("/tmp/skynet-zdotdir");
  ...
  describe("terminal title", () => {
    it.skipIf(!hasZsh)("restores the user's ZDOTDIR through the zsh wrapper", async () => {
      ...
      writeFileSync(join(realZdotdir, ".zshenv"), "export SKYNET_TEST_REAL_ZDOTDIR=1\n");

      const session = trackSession(
        await createTerminal({
          cwd: homeDir,
          command: "/bin/zsh",
          args: ["-c", 'printf \'%s\\n%s\\n\' "${ZDOTDIR-}" "${SKYNET_TEST_REAL_ZDOTDIR-}"'],
          env: {
            HOME: homeDir,
            ZDOTDIR: realZdotdir,
          },
