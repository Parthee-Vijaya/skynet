...
import type { SkynetDaemonConfig } from "./bootstrap.js";
...
const DEFAULT_RELAY_ENDPOINT = "relay.skynet.sh:443";
const DEFAULT_APP_BASE_URL = "https://app.skynet.sh";
...
export function loadConfig(
  ...
  const persisted = loadPersistedConfig(skynetHome);

  // SKYNET_LISTEN can be:
  // - host:port (TCP)
  ...
  const envCorsOrigins = env.SKYNET_CORS_ORIGINS
    ? env.SKYNET_CORS_ORIGINS.split(",").map((s) => s.trim())
    : [];
  ...
  const hostnames = mergeHostnames([
    ...
    parseHostnamesEnv(env.SKYNET_HOSTNAMES ?? env.SKYNET_ALLOWED_HOSTS),
  ...
  const relayEnabled =
    ...
    parseBooleanEnv(env.SKYNET_RELAY_ENABLED) ??
    persisted.daemon?.relay?.enabled ??
    true;

  const relayEndpoint =
    env.SKYNET_RELAY_ENDPOINT ?? persisted.daemon?.relay?.endpoint ?? DEFAULT_RELAY_ENDPOINT;

  const relayPublicEndpoint =
    env.SKYNET_RELAY_PUBLIC_ENDPOINT ?? persisted.daemon?.relay?.publicEndpoint ?? relayEndpoint;

  const appBaseUrl = env.SKYNET_APP_BASE_URL ?? persisted.app?.baseUrl ?? DEFAULT_APP_BASE_URL;
  ...
  const envVoiceLlmProvider = parseOptionalVoiceLlmProvider(env.SKYNET_VOICE_LLM_PROVIDER);
