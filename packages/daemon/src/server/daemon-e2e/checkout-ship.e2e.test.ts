...
function initGitRepo(repoDir: string): void {
  ...
  execSync("git config user.email 'skynet-test@example.com'", {
    cwd: repoDir,
    stdio: "pipe",
  });
  execSync("git config user.name 'Skynet Test'", {
    cwd: repoDir,
    stdio: "pipe",
  });
...
function createTempRepoName(): string {
  ...
  return `skynet-checkout-ship-${Date.now()}-${rand}`;
}
    ...
    async () => {
      ...
      try {
        ...
        const worktree = await createLegacyWorktreeForTest({
          ...
          skynetHome: ctx.daemon.skynetHome,
        });
        ...
        const worktreeList = await ctx.client.getSkynetWorktreeList({
          cwd: repoDir,
        });
        ...
        const archiveResult = await ctx.client.archiveSkynetWorktree({
          worktreePath: worktree.worktreePath,
        });
        expect(archiveResult.error).toBeNull();
        expect(archiveResult.success).toBe(true);

        const worktreeListAfter = await ctx.client.getSkynetWorktreeList({
          cwd: repoDir,
        });
        expect(
