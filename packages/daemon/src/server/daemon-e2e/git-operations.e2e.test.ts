...
describe("daemon E2E", () => {
  ...
  describe("worktree setup", () => {
    test("runs skynet.json setup asynchronously and reports status via timeline tool_call", async () => {
      ...
      const setupCommand =
        'while [ ! -f "$SKYNET_WORKTREE_PATH/allow-setup" ]; do sleep 0.05; done; echo "done" > "$SKYNET_WORKTREE_PATH/setup-done.txt"';
      writeFileSync(
        path.join(repoRoot, "skynet.json"),
        JSON.stringify({ worktree: { setup: [setupCommand] } }),
      );
      execSync("git add skynet.json", { cwd: repoRoot, stdio: "pipe" });
      execSync("git -c commit.gpgsign=false commit -m 'add skynet.json'", {
        cwd: repoRoot,
        stdio: "pipe",
      });
      ...
      expect(agent.cwd).toContain(path.join(".skynet", "worktrees"));
      ...
      const completed = await waitForTimelineToolCall(
        collector.messages,
        agent.id,
        (item) => item.name === "skynet_worktree_setup" && item.status === "completed",
        20000,
      );
    ...
    test("bootstraps configured worktree terminals after setup succeeds", async () => {
      await withShell("/bin/sh", async () => {
        ...
        const setupCommand =
          'while [ ! -f "$SKYNET_WORKTREE_PATH/allow-setup" ]; do sleep 0.05; done; echo "done" > "$SKYNET_WORKTREE_PATH/setup-done.txt"; echo "$SKYNET_WORKTREE_PORT" > "$SKYNET_WORKTREE_PATH/setup-port.txt"';
        writeFileSync(
          path.join(repoRoot, "skynet.json"),
          JSON.stringify({
            worktree: {
        ...
        const terminalsBootstrapToolCall = await waitForTimelineToolCall(
          collector.messages,
          agent.id,
          (item) => item.name === "skynet_worktree_terminals" && item.status === "completed",
          30000,
        );
        ...
        ctx.client.sendTerminalInput(manualTerminalId, {
          type: "input",
          data: 'echo "$SKYNET_WORKTREE_PORT" > "$SKYNET_WORKTREE_PATH/manual-terminal-port.txt"\r',
        });
        await waitForPathExists({
    ...
    test("reports failures via timeline tool_call without deleting the created worktree", async () => {
      ...
      const setupCommand =
        'echo "started" > "$SKYNET_WORKTREE_PATH/setup-start.txt"; sleep 0.1; echo "boom" 1>&2; exit 7';
      writeFileSync(
        path.join(repoRoot, "skynet.json"),
        JSON.stringify({
          worktree: {
      ...
      const started = await waitForTimelineToolCall(
        collector.messages,
        agent.id,
        (item) => item.name === "skynet_worktree_setup" && item.status === "running",
        10000,
      );
  ...
  describe("createAgent with worktree", () => {
    test("creates agent in ~/.skynet/worktrees/{hash} when worktree is requested", async () => {
      const cwd = tmpCwd();
      ...
      expect(realpathSync(agent.cwd)).toBe(
        realpathSync(path.join(ctx.daemon.skynetHome, "worktrees", projectHash, "worktree-test")),
      );
      expect(existsSync(agent.cwd)).toBe(true);
  ...
  describe("archiveSkynetWorktree", () => {
    test("archives worktree by running teardown commands and shutting down worktree terminals", async () => {
      ...
      writeFileSync(
        ...
        JSON.stringify({
          worktree: {
            ...
            teardown: [`echo "$SKYNET_WORKTREE_PATH" > "${teardownMarkerPath}"`],
          },
        }),
      );
      execSync("git add skynet.json", { cwd: repoRoot, stdio: "pipe" });
      ...
      const archive = await ctx.client.archiveSkynetWorktree({
