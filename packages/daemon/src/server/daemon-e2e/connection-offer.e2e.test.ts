...
describe("ConnectionOfferV2 (daemon E2E)", () => {
  ...
  test("emits relay-only offer URL with stable serverId", async () => {
    process.env.SKYNET_PRIMARY_LAN_IP = "192.168.1.12";
    ...
    try {
      ...
      expect(offerUrl.startsWith("https://app.skynet.sh/#offer=")).toBe(true);
      ...
      expect(offer.relay.endpoint).toBe("relay.skynet.sh:443");
  ...
  test("persists serverId and daemon keypair across daemon restarts", async () => {
    ...
    const tempHomeRoot = await mkdtemp(path.join(os.tmpdir(), "skynet-offer-home-"));

    const { logger: logger1 } = createCapturingLogger();
    const daemon1 = await createTestSkynetDaemon({
      listen: "0.0.0.0",
      logger: logger1,
      relayEnabled: true,
      skynetHomeRoot: tempHomeRoot,
      cleanup: false,
    });
    ...
    try {
      ...
      const daemon2 = await createTestSkynetDaemon({
        listen: "0.0.0.0",
        logger: logger2,
        relayEnabled: true,
        skynetHomeRoot: tempHomeRoot,
  ...
  test("respects --no-relay (CLI) by not emitting a pairing offer", async () => {
    ...
    const tempHome = await mkdtemp(path.join(os.tmpdir(), "skynet-offer-e2e-"));
    ...
    const env = {
      ...process.env,
      SKYNET_HOME: tempHome,
      SKYNET_LISTEN: `0.0.0.0:${port}`,
      OPENAI_API_KEY: "",
      SKYNET_DICTATION_ENABLED: "0",
      SKYNET_VOICE_MODE_ENABLED: "0",
