...
import { isSkynetDictationDebugEnabled } from "./agent/recordings-debug.js";
...
import {
  ...
  type CreateSkynetWorktreeInput,
  type CreateSkynetWorktreeResult,
...
import {
  ...
  handleCreateSkynetWorktreeRequest as handleCreateWorktreeRequest,
  handleSkynetWorktreeArchiveRequest as handleWorktreeArchiveRequest,
  handleSkynetWorktreeListRequest as handleWorktreeListRequest,
  handleWorkspaceSetupStatusRequest as handleWorkspaceSetupStatusRequestMessage,
} from "./worktree-session.js";
import { killTerminalsUnderPath as killWorktreeTerminalsUnderPath } from "./skynet-worktree-archive-service.js";
...
export class Session {
  ...
  private readonly skynetHome: string;
    ...
    const checkout =
      project.kind !== "git"
        ? {
            ...
            remoteUrl: null,
            worktreeRoot: null,
            isSkynetOwnedWorktree: false as const,
            mainRepoRoot: null,
          }
        : workspace.kind === "worktree"
          ? {
              ...
              isSkynetOwnedWorktree: true as const,
    ...
    try {
      ...
      try {
        switch (msg.type) {
          ...
          case "skynet_worktree_list_request":
            await this.handleSkynetWorktreeListRequest(msg);
            break;

          case "skynet_worktree_archive_request":
            await this.handleSkynetWorktreeArchiveRequest(msg);
            break;

          case "create_skynet_worktree_request":
            await this.handleCreateSkynetWorktreeRequest(msg);
            break;
    ...
    return buildWorktreeAgentSessionConfig(
      {
        ...
        workspaceGitService: this.workspaceGitService,
        createSkynetWorktree: (input, serviceOptions) =>
          this.createSkynetWorktree(input, serviceOptions),
    ...
    try {
      ...
      if (
        ...
        return {
          ...
          body: "Automated PR generated by Skynet.",
        };
      }
      throw error;
    }
  ...
  private buildCheckoutStatusPayloadFromSnapshot({
    ...
    if (snapshot.git.isSkynetOwnedWorktree) {
  ...
  private static readonly SKYNET_STASH_PREFIX = "skynet-auto-stash:";
  ...
  private async handleStashSaveRequest(
    ...
    try {
      ...
      const message = branchLabel
        ? `${Session.SKYNET_STASH_PREFIX} ${branchLabel}`
        : `${Session.SKYNET_STASH_PREFIX} unnamed`;
  ...
  private async handleStashListRequest(
    ...
    const { cwd, requestId } = msg;
    const skynetOnly = msg.skynetOnly !== false;
    try {
      const entries = await this.workspaceGitService.listStashes(cwd, { skynetOnly });
  ...
  private async handleSkynetWorktreeListRequest(
    msg: Extract<SessionInboundMessage, { type: "skynet_worktree_list_request" }>,
  ): Promise<void> {
  ...
  private async handleSkynetWorktreeArchiveRequest(
    msg: Extract<SessionInboundMessage, { type: "skynet_worktree_archive_request" }>,
  ): Promise<void> {
  ...
  private buildWorkspaceGitRuntimePayload(
    ...
    return {
      ...
      remoteUrl: snapshot.git.remoteUrl,
      isSkynetOwnedWorktree: snapshot.git.isSkynetOwnedWorktree,
      isDirty: snapshot.git.isDirty,
  ...
  private async describeCreatedWorktreeWorkspace(
    result: CreateSkynetWorktreeResult,
  ): Promise<WorkspaceDescriptorPayload> {
  ...
  private async createSkynetWorktree(
    input: CreateSkynetWorktreeInput,
    options?: {
      resolveDefaultBranch?: (repoRoot: string) => Promise<string>;
    },
  ): Promise<CreateSkynetWorktreeResult> {
    const coreDeps = createWorktreeCoreDeps(this.github);
    const result = await createSkynetWorktree(input, {
      ...coreDeps,
  ...
  private async handleCreateSkynetWorktreeRequest(
    request: Extract<SessionInboundMessage, { type: "create_skynet_worktree_request" }>,
  ): Promise<void> {
    return handleCreateWorktreeRequest(
      {
        ...
        emit: (message) => this.emit(message),
        createSkynetWorktree: (input) => this.createSkynetWorktree(input),
  ...
  private async handleArchiveWorkspaceRequest(
    ...
    try {
      ...
      if (existing.kind === "worktree") {
        throw new Error("Use worktree archive for Skynet worktrees");
      }
      const archivedAt = new Date().toISOString();
  ...
  private emit(msg: SessionOutboundMessage): void {
    ...
    if (
      ...
      (process.env.TTS_DEBUG_AUDIO_DIR || isSkynetDictationDebugEnabled()) &&
      msg.payload.groupId &&
