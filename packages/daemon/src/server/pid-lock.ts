...
function getPidFilePath(skynetHome: string): string {
  return join(skynetHome, "skynet.pid");
}
...
export async function acquirePidLock(
  ...
  const pidPath = getPidFilePath(skynetHome);

  // Ensure skynetHome directory exists
  ...
  if (existingLock) {
    if (isPidRunning(existingLock.pid)) {
      ...
      throw new PidLockError(
        `Another Skynet daemon is already running (PID ${existingLock.pid}, started ${existingLock.startedAt})`,
        existingLock,
      );
    }
  ...
  const lockInfo: PidLockInfo = {
    ...
    listen,
    ...(process.env.SKYNET_DESKTOP_MANAGED === "1" ? { desktopManaged: true } : {}),
  };
  ...
  try {
    ...
    if (err.code === "EEXIST") {
      ...
      try {
        ...
        throw new PidLockError(
          `Another Skynet daemon is already running (PID ${raceLock.pid})`,
          raceLock,
        );
      } catch (innerErr) {
...
export async function getPidLockInfo(skynetHome: string): Promise<PidLockInfo | null> {
  const pidPath = getPidFilePath(skynetHome);
  try {
    const content = await readFile(pidPath, "utf-8");
...
export async function isLocked(
  ...
  const info = await getPidLockInfo(skynetHome);
  if (!info) {
    return { locked: false };
  }
