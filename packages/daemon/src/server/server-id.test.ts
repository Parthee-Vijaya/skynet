...
function tmpHome(): string {
  return mkdtempSync(path.join(tmpdir(), "skynet-server-id-"));
}
...
describe("getOrCreateServerId", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.SKYNET_SERVER_ID;
    home = tmpHome();
  });
  ...
  it("creates and persists a stable id per SKYNET_HOME", () => {
  ...
  it("respects and persists SKYNET_SERVER_ID override", () => {
    process.env.SKYNET_SERVER_ID = "test-daemon-id";
