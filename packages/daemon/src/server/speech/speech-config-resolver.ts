...
import type { SkynetOpenAIConfig, SkynetSpeechConfig } from "../bootstrap.js";
...
function resolveRequestedSpeechProviders(params: {
  ...
  const voiceModeEnabled = resolveOptionalBooleanFlag(
    params.env.SKYNET_VOICE_MODE_ENABLED ?? params.persisted.features?.voiceMode?.enabled,
  );
  const featureProviders = {
    dictationStt: {
      configuredValue:
        params.env.SKYNET_DICTATION_STT_PROVIDER ??
        params.persisted.features?.dictation?.stt?.provider,
      enabled: resolveOptionalBooleanFlag(
        params.env.SKYNET_DICTATION_ENABLED ?? params.persisted.features?.dictation?.enabled,
      ),
    },
    voiceTurnDetection: {
      configuredValue:
        params.env.SKYNET_VOICE_TURN_DETECTION_PROVIDER ??
        params.persisted.features?.voiceMode?.turnDetection?.provider,
      enabled: voiceModeEnabled,
    },
    voiceStt: {
      configuredValue:
        params.env.SKYNET_VOICE_STT_PROVIDER ?? params.persisted.features?.voiceMode?.stt?.provider,
      enabled: voiceModeEnabled,
    },
    voiceTts: {
      configuredValue:
        params.env.SKYNET_VOICE_TTS_PROVIDER ?? params.persisted.features?.voiceMode?.tts?.provider,
      enabled: voiceModeEnabled,
    },
  } satisfies Record<
...
export function resolveSpeechConfig(params: {
  ...
  openai: SkynetOpenAIConfig | undefined;
