...
function shouldIncludeLocalProviderConfig(params: {
  ...
  return (
    ...
    params.env.SKYNET_LOCAL_MODELS_DIR !== undefined ||
...
export function resolveLocalSpeechConfig(params: {
  ...
  const parsed = LocalSpeechResolutionSchema.parse({
    ...
    modelsDir:
      params.env.SKYNET_LOCAL_MODELS_DIR ??
      params.persisted.providers?.local?.modelsDir ??
      path.join(params.skynetHome, DEFAULT_LOCAL_MODELS_SUBDIR),
    dictationLocalSttModel:
      params.env.SKYNET_DICTATION_LOCAL_STT_MODEL ??
      ...
      DEFAULT_LOCAL_STT_MODEL,
    voiceLocalSttModel:
      params.env.SKYNET_VOICE_LOCAL_STT_MODEL ??
      persistedLocalFeatureModel(
      ...
      DEFAULT_LOCAL_STT_MODEL,
    voiceLocalTtsModel:
      params.env.SKYNET_VOICE_LOCAL_TTS_MODEL ??
      persistedLocalFeatureModel(
    ...
    voiceLocalTtsSpeakerId:
      params.env.SKYNET_VOICE_LOCAL_TTS_SPEAKER_ID ??
      params.persisted.features?.voiceMode?.tts?.speakerId,
    voiceLocalTtsSpeed:
      params.env.SKYNET_VOICE_LOCAL_TTS_SPEED ?? params.persisted.features?.voiceMode?.tts?.speed,
  });
