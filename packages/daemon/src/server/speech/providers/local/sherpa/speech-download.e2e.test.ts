...
describe("speech models (download E2E)", () => {
  downloadTest(
    ...
    async () => {
      ...
      const skynetHomeRoot = mkdtempSync(path.join(tmpdir(), "skynet-speech-download-"));
      const modelsDir = path.join(skynetHomeRoot, ".skynet", "models", "local-speech");
