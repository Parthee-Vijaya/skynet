...
describe("resolveSpeechConfig", () => {
  test("resolves local-first defaults without env overrides", () => {
    const skynetHome = "/tmp/skynet-home";
    ...
    expect(result.speech.local).toEqual({
      modelsDir: path.join(skynetHome, "models", "local-speech"),
      models: {
  ...
  test("resolves feature-scoped local model env vars", () => {
    ...
    const env = {
      SKYNET_DICTATION_LOCAL_STT_MODEL: "zipformer",
      SKYNET_VOICE_LOCAL_STT_MODEL: "parakeet",
      SKYNET_VOICE_LOCAL_TTS_MODEL: "kitten",
      SKYNET_VOICE_LOCAL_TTS_SPEAKER_ID: "5",
      SKYNET_VOICE_LOCAL_TTS_SPEED: "1.35",
      SKYNET_LOCAL_MODELS_DIR: "/tmp/models",
      OPENAI_API_KEY: "env-key",
      SKYNET_VOICE_STT_PROVIDER: "openai",
      SKYNET_DICTATION_STT_PROVIDER: "local",
      SKYNET_VOICE_TTS_PROVIDER: "local",
  ...
  test("ignores deprecated shared local model env vars", () => {
    const persisted = PersistedConfigSchema.parse({});
    const env = {
      SKYNET_LOCAL_STT_MODEL: "zipformer-bilingual-zh-en-2023-02-20",
      SKYNET_LOCAL_TTS_MODEL: "kitten-nano-en-v0_1-fp16",
    } as NodeJS.ProcessEnv;
