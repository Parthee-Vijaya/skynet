...
export function deriveProjectGroupingKey(options: {
  ...
  if (options.isSkynetOwnedWorktree && mainRepoRoot) {
    return mainRepoRoot;
  }
...
export function deriveProjectRootPath(input: {
  ...
  if (input.checkout.isGit && input.checkout.isSkynetOwnedWorktree) {
...
export function deriveWorkspaceKind(checkout: ProjectCheckoutLitePayload): PersistedWorkspaceKind {
  ...
  return checkout.isSkynetOwnedWorktree ? "worktree" : "local_checkout";
}
...
export function checkoutLiteFromGitSnapshot(
  ...
  if (git.isSkynetOwnedWorktree && git.mainRepoRoot) {
    return {
      cwd,
      isGit: true,
...
export async function buildProjectPlacementForCwd(input: {
  ...
  const projectKey = deriveProjectGroupingKey({
    ...
    isSkynetOwnedWorktree: checkout.isSkynetOwnedWorktree,
