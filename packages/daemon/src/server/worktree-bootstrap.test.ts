...
describe("runAsyncWorktreeBootstrap", () => {
  ...
  it("truncates each command output to 64kb in the middle", async () => {
    ...
    const persistedSetupItem = persisted.find(
      (item): item is Extract<AgentTimelineItem, { type: "tool_call" }> =>
        item.type === "tool_call" && item.name === "skynet_worktree_setup",
    );
  ...
  it("shares the same worktree runtime port across setup and bootstrap terminals", async () => {
    writeFileSync(
      ...
      JSON.stringify({
        worktree: {
          setup: ['echo "$SKYNET_WORKTREE_PORT" > setup-port.txt'],
          terminals: [
            {
              name: "Port Terminal",
              command: "true",
    ...
    expect(registeredEnvs[0]?.env.SKYNET_WORKTREE_PORT).toBe(setupPort);
    expect(createTerminalEnvs.length).toBeGreaterThan(0);
    expect(createTerminalEnvs[0]?.SKYNET_WORKTREE_PORT).toBe(setupPort);

    const terminalToolCall = persisted.find(
      (item): item is Extract<AgentTimelineItem, { type: "tool_call" }> =>
        item.type === "tool_call" &&
        item.name === "skynet_worktree_terminals" &&
  ...
  function commitSkynetScripts(
    scripts: Record<string, { command: string; type?: "script" | "service" }>,
    message = "add script config",
  ): void {
    writeFileSync(join(repoDir, "skynet.json"), JSON.stringify({ scripts }));
    execSync("git add skynet.json", { cwd: repoDir, stdio: "pipe" });
  ...
  it("spawns services with route registration and injected peer service env vars", async () => {
    ...
    expect(createTerminalCalls[0]?.env?.SKYNET_PORT).toEqual(expect.any(String));
    expect(createTerminalCalls[0]?.env?.HOST).toBe("127.0.0.1");
    expect(createTerminalCalls[0]?.env?.SKYNET_URL).toBe(
      "http://api.feature-socket-service.repo.localhost:6767",
    );
    expect(createTerminalCalls[0]?.env?.SKYNET_SERVICE_API_PORT).toBe(
      createTerminalCalls[0]?.env?.SKYNET_PORT,
    );
    expect(createTerminalCalls[0]?.env?.SKYNET_SERVICE_API_URL).toBe(
      "http://api.feature-socket-service.repo.localhost:6767",
    );
    ...
    expect(createTerminalCalls[0]?.env?.SKYNET_SERVICE_APP_SERVER_PORT).toBe(
      String(plannedAppServerPort),
    );
    expect(createTerminalCalls[0]?.env?.SKYNET_SERVICE_APP_SERVER_URL).toBe(
  ...
  it("refreshes a stopped service port on respawn and updates the route", async () => {
    ...
    expect(createTerminalCalls[2]?.env?.SKYNET_SERVICE_WORKER_PORT).toBe(String(workerPort));
  ...
  it("fails normalized service env name collisions before terminal creation", async () => {
    ...
    expect(createTerminalCalls).toHaveLength(1);
    expect(createTerminalCalls[0]?.env).toHaveProperty("SKYNET_SERVICE_APP_SERVER_PORT");
    expect(createTerminalCalls[0]?.env).toHaveProperty("SKYNET_SERVICE_WORKER_PORT");
  });
  ...
  it("injects real peer service env into terminal-backed services", async () => {
    ...
    expect(apiEnv.SKYNET_SERVICE_API_URL).toBe("http://api.feature-peer-env.repo.localhost:6767");
    expect(apiEnv.SKYNET_SERVICE_WEB_URL).toBe("http://web.feature-peer-env.repo.localhost:6767");
    expect(apiEnv.SKYNET_SERVICE_API_PORT).toEqual(expect.stringMatching(/^\d+$/));
    expect(apiEnv.SKYNET_SERVICE_WEB_PORT).toEqual(expect.stringMatching(/^\d+$/));
    expect(apiEnv.SKYNET_URL).toBe(apiEnv.SKYNET_SERVICE_API_URL);
    expect(apiEnv.SKYNET_PORT).toBe(apiEnv.SKYNET_SERVICE_API_PORT);
    expect(apiEnv).not.toHaveProperty("PORT");

    expect(webEnv.SKYNET_SERVICE_API_URL).toBe("http://api.feature-peer-env.repo.localhost:6767");
    expect(webEnv.SKYNET_SERVICE_WEB_URL).toBe("http://web.feature-peer-env.repo.localhost:6767");
    expect(webEnv.SKYNET_SERVICE_API_PORT).toBe(apiEnv.SKYNET_SERVICE_API_PORT);
    expect(webEnv.SKYNET_SERVICE_WEB_PORT).toBe(apiEnv.SKYNET_SERVICE_WEB_PORT);
    expect(webEnv.SKYNET_URL).toBe(webEnv.SKYNET_SERVICE_WEB_URL);
    expect(webEnv.SKYNET_PORT).toBe(webEnv.SKYNET_SERVICE_WEB_PORT);
    expect(webEnv).not.toHaveProperty("PORT");

    const apiPort = Number(apiEnv.SKYNET_SERVICE_API_PORT);
    const webPort = Number(apiEnv.SKYNET_SERVICE_WEB_PORT);
