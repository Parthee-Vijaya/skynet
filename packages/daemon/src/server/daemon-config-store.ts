...
export class DaemonConfigStore {
  ...
  constructor(skynetHome: string, initial: MutableDaemonConfig, logger?: LoggerLike) {
  ...
  private persistConfig(config: MutableDaemonConfig): void {
    const persisted = loadPersistedConfig(this.skynetHome, this.logger);
    const nextPersisted = mergeMutableConfigIntoPersistedConfig({
      persisted,
      mutable: config,
    });
    savePersistedConfig(this.skynetHome, nextPersisted, this.logger);
  }
}
