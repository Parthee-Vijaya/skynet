...
function createService(options?: {
  ...
  listSkynetWorktrees?: ReturnType<typeof vi.fn>;
  ...
  return new WorkspaceGitServiceImpl({
    ...
    deps: {
      ...
      listSkynetWorktrees: options?.listSkynetWorktrees ?? vi.fn(async () => []),
...
describe("WorkspaceGitServiceImpl D2 read methods", () => {
  ...
  test("listStashes cold-loads, warms, forces, and coalesces per cwd", async () => {
    let nowMs = 0;
    const stashOutput = "stash@{0}\u0000skynet-auto-stash: feature\n";
    ...
    const first = service.listStashes("/tmp/repo", { skynetOnly: true });
    const second = service.listStashes("/tmp/repo/.", { skynetOnly: true });
    await flushPromises();
    ...
    await expect(Promise.all([first, second])).resolves.toEqual([
      [{ index: 0, message: "skynet-auto-stash: feature", branch: "feature", isSkynet: true }],
      [{ index: 0, message: "skynet-auto-stash: feature", branch: "feature", isSkynet: true }],
    ]);

    nowMs = 1_000;
    await service.listStashes("/tmp/repo", { skynetOnly: true });
    expect(runGitCommand).toHaveBeenCalledTimes(1);

    await service.listStashes("/tmp/repo", { skynetOnly: true }, { force: true, reason: "test" });
    expect(runGitCommand).toHaveBeenCalledTimes(2);
  ...
  test("listWorktrees cold-loads, warms, forces, and coalesces per repo root", async () => {
    ...
    const listSkynetWorktrees = vi.fn().mockResolvedValue(worktrees);
    const service = createService({
      listSkynetWorktrees,
      now: () => new Date(nowMs),
    });
    ...
    expect(listSkynetWorktrees).toHaveBeenCalledTimes(1);
    ...
    expect(listSkynetWorktrees).toHaveBeenCalledTimes(2);
  ...
  test("listWorktrees shares one repo-root scoped read across sibling workspace cwds", async () => {
    ...
    const worktrees = [
      {
        path: join(tempDir, "skynet-home", "worktrees", "repo", "feature"),
        createdAt: "2026-04-12T00:00:00.000Z",
        branchName: "feature",
      },
    ];
    const listSkynetWorktrees = vi.fn(async () => worktrees);
    const service = createService({
    ...
    try {
      ...
      expect(listSkynetWorktrees).toHaveBeenCalledTimes(1);
      expect(listSkynetWorktrees).toHaveBeenCalledWith({
        cwd: repoDir,
        skynetHome: "/tmp/skynet-test",
      });
    } finally {
  ...
  test("resolveRepoRemoteUrl reads remote URL through the snapshot cache", async () => {
    ...
    await expect(service.resolveRepoRemoteUrl("/tmp/repo")).resolves.toBe(
      "https://github.com/getskynet/skynet.git",
    );
    nowMs = 1_000;
    await expect(service.resolveRepoRemoteUrl("/tmp/repo/.")).resolves.toBe(
      "https://github.com/getskynet/skynet.git",
  ...
  test("getWorkspaceGitMetadata derives reconciliation metadata from the snapshot cache", async () => {
    ...
    await expect(
      ...
      gitRemote: "https://github.com/getskynet/skynet.git",
      isWorktree: false,
      projectSlug: "skynet",
      repoRoot: "/tmp/repo",
      currentBranch: "feature/service-metadata",
