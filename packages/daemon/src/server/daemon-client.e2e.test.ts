...
const localModelsDir =
  process.env.SKYNET_LOCAL_MODELS_DIR ?? path.join(homedir(), ".skynet", "models", "local-speech");
...
describe("daemon client E2E", () => {
    ...
    const speechConfig = hasLocalSpeech
      ? {
          ...
          local: {
            ...
            models: {
              dictationStt:
                process.env.SKYNET_DICTATION_LOCAL_STT_MODEL ??
                "zipformer-bilingual-zh-en-2023-02-20",
              voiceStt:
                process.env.SKYNET_VOICE_LOCAL_STT_MODEL ?? "zipformer-bilingual-zh-en-2023-02-20",
              voiceTts: process.env.SKYNET_VOICE_LOCAL_TTS_MODEL ?? "kitten-nano-en-v0_1-fp16",
            },
          },
        }
      : openaiApiKey
  ...
  test("returns home-scoped directory suggestions", async () => {
    const insideHomeDir = mkdtempSync(path.join(homedir(), "skynet-dir-suggestion-"));
    const outsideHomeDir = mkdtempSync(path.join(tmpdir(), "skynet-dir-suggestion-outside-"));
