...
function createGitRepo(): { tempDir: string; repoDir: string; skynetHome: string } {
  ...
  return { tempDir, repoDir, skynetHome };
}

function createGitRepoWithDevBranch(): { tempDir: string; repoDir: string; skynetHome: string } {
  const { tempDir, repoDir, skynetHome } = createGitRepo();
...
function createGitHubPrRemoteRepo(): { tempDir: string; repoDir: string; skynetHome: string } {
  const { tempDir, repoDir, skynetHome } = createGitRepo();
...
function createForkGitHubPrRemoteRepo(): {
  ...
  return { tempDir, repoDir, headRemoteDir, skynetHome };
}
...
describe.skipIf(process.platform === "win32")("createWorktreeCore", () => {
  ...
  test("checks out the legacy RPC GitHub PR attachment branch", async () => {
    const { tempDir, repoDir, skynetHome } = createGitHubPrRemoteRepo();
    cleanupPaths.push(tempDir);
  ...
  test("branches off an explicit refName base", async () => {
    const { tempDir, repoDir, skynetHome } = createGitRepoWithDevBranch();
    cleanupPaths.push(tempDir);
  ...
  test("checks out a fork PR whose head branch collides with local main", async () => {
    const { tempDir, repoDir, headRemoteDir, skynetHome } = createForkGitHubPrRemoteRepo();
    cleanupPaths.push(tempDir);
    const github = {
  ...
  test("uses a unique local branch when the same fork PR branch already exists", async () => {
    ...
    expect(
      execSync("git config --get remote.skynet-pr-526.push", {
  ...
  test("reuses an existing branch-off worktree for the same slug", async () => {
    ...
    const first = await createCoreWorktree(
      { cwd: repoDir, worktreeSlug: "reused-worktree", skynetHome, runSetup: false },
      deps,
    );
    const second = await createCoreWorktree(
      { cwd: repoDir, worktreeSlug: "reused-worktree", skynetHome, runSetup: false },
...
describe("resolveWorktreeRepoRoot", () => {
  test("resolves repository roots through the workspace git service", async () => {
    ...
    await expect(
      resolveWorktreeRepoRoot(
        { cwd: "/tmp/main-repo/worktrees/feature", skynetHome: "/tmp/skynet-home" },
        workspaceGitService,
      ),
