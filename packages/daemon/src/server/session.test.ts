...
const skynetWorktreeServiceMocks = vi.hoisted(() => ({
  createSkynetWorktree: vi.fn(),
}));
...
vi.mock("./skynet-worktree-service.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("./skynet-worktree-service.js")>();
  return {
    ...actual,
    createSkynetWorktree: skynetWorktreeServiceMocks.createSkynetWorktree,
  };
});
...
function createWorkspaceGitSnapshot(
  ...
  return {
    ...
    git: {
      ...
      remoteUrl: "https://github.com/getskynet/skynet.git",
      ...
      checks: [
        {
          name: "typecheck",
          status: "success",
          url: "https://github.com/getskynet/skynet/actions/runs/1",
          workflow: "CI",
          duration: "1m 20s",
        },
      ],
...
describe("session checkout pull request creation", () => {
  ...
  test("forces workspace git and GitHub refresh after creating a pull request", async () => {
    ...
    checkoutGitMocks.createPullRequest.mockResolvedValue({
      url: "https://github.com/getskynet/skynet/pull/2",
      number: 2,
    });
    const session = createSessionForTest({ github, workspaceGitService, messages });
...
describe("session branch validation", () => {
  ...
  test("does not validate tags as branches", async () => {
    const tempDir = mkdtempSync(join(tmpdir(), "skynet-session-branch-validation-"));
...
describe("session stash list handling", () => {
  test("lists stashes through the workspace git service", async () => {
    ...
    const entries = [
      {
        index: 0,
        message: "skynet-auto-stash: feature",
        branch: "feature",
        isSkynet: true,
      },
    ];
...
describe("session skynet worktree creation handling", () => {
  test("forces workspace git refreshes for the source repo and created worktree", async () => {
    const workspaceGitService = { getSnapshot: vi.fn().mockResolvedValue({}) };
    const session = createSessionForTest({ workspaceGitService });
    skynetWorktreeServiceMocks.createSkynetWorktree.mockResolvedValue({
      repoRoot: "/tmp/repo",
      worktree: {
        branchName: "feature/new-worktree",
        worktreePath: "/tmp/skynet/worktrees/new-worktree",
      },
      workspace: {
        workspaceId: "workspace-new-worktree",
        projectId: "project-repo",
        cwd: "/tmp/skynet/worktrees/new-worktree",
        kind: "worktree",
    ...
    await (session as any).createSkynetWorktree({
      cwd: "/tmp/repo",
    ...
    expect(workspaceGitService.getSnapshot).toHaveBeenCalledWith(
      "/tmp/skynet/worktrees/new-worktree",
      {
        force: true,
        reason: "create-worktree",
      },
  ...
  test("passes service-owned git metadata into workspace script spawning", async () => {
    ...
    const workspaceGitService = {
      ...
      getWorkspaceGitMetadata: vi.fn().mockResolvedValue({
        projectKind: "git",
        projectDisplayName: "getskynet/skynet",
...
describe("session pull request timeline handling", () => {
        ...
        items: [
          {
            ...
            number: 42,
            title: "Ship search",
            url: "https://github.com/getskynet/skynet/pull/42",
            state: "OPEN",
            body: null,
            labels: [],
            baseRefName: "main",
        ...
        items: [
          {
            ...
            url: "https://github.com/getskynet/skynet/pull/42#pullrequestreview-1",
  ...
  test.each([
    { prNumber: 0, repoOwner: "getskynet", repoName: "skynet" },
    { prNumber: -1, repoOwner: "getskynet", repoName: "skynet" },
    { prNumber: 42, repoOwner: "get skynet", repoName: "skynet" },
    { prNumber: 42, repoOwner: "getskynet/cli", repoName: "skynet" },
    { prNumber: 42, repoOwner: "get$skynet", repoName: "skynet" },
    { prNumber: 42, repoOwner: "getskynet", repoName: "pa seo" },
    { prNumber: 42, repoOwner: "getskynet", repoName: "skynet/app" },
    { prNumber: 42, repoOwner: "getskynet", repoName: "skynet!" },
