...
import type {
  CreateSkynetWorktreeInput,
  CreateSkynetWorktreeResult,
} from "./skynet-worktree-service.js";
import {
  archiveSkynetWorktree,
  type ArchiveSkynetWorktreeDependencies,
} from "./skynet-worktree-archive-service.js";
...
type CreateSkynetWorktreeInBackgroundDependencies = {
  skynetHome?: string;
...
type HandleCreateSkynetWorktreeRequestDependencies = {
  skynetHome?: string;
  describeWorkspaceRecord: (
    result: CreateSkynetWorktreeResult,
  ) => Promise<WorkspaceDescriptorPayload>;
  emit: EmitSessionMessage;
  createSkynetWorktree: (input: CreateSkynetWorktreeInput) => Promise<CreateSkynetWorktreeResult>;
...
export async function buildAgentSessionConfig(
  ...
  if (normalized.createWorktree) {
    ...
    const createdWorktree = await dependencies.createSkynetWorktree(
      {
        cwd,
        ...
        attachments,
        runSetup: false,
        skynetHome: dependencies.skynetHome,
      },
      {
...
export async function handleSkynetWorktreeListRequest(
  dependencies: {
    emit: EmitSessionMessage;
    skynetHome?: string;
    workspaceGitService: WorkspaceGitService;
...
export async function handleSkynetWorktreeArchiveRequest(
  dependencies: Omit<
    ArchiveSkynetWorktreeDependencies,
  ...
  try {
    if (!targetPath) {
      ...
      if (!match) {
        throw new Error(`Skynet worktree not found for branch ${msg.branchName}`);
      }
    ...
    const ownership = await isSkynetOwnedWorktreeCwd(targetPath, {
      skynetHome: dependencies.skynetHome,
    });
    if (!ownership.allowed) {
      dependencies.emit({
        type: "skynet_worktree_archive_response",
        payload: {
          success: false,
          removedAgents: [],
          error: {
            code: "NOT_ALLOWED",
            message: "Worktree is not a Skynet-owned worktree",
          },
          requestId,
        },
      });
    ...
    // check already proved the path lives under $SKYNET_HOME/worktrees.
    repoRoot = ownership.repoRoot ?? repoRoot ?? null;

    const removedAgents = await archiveSkynetWorktree(dependencies, {
      targetPath,
      repoRoot,
...
export async function handleCreateSkynetWorktreeRequest(
  dependencies: HandleCreateSkynetWorktreeRequestDependencies,
  ...
  try {
    ...
    dependencies.emit({
      type: "create_skynet_worktree_response",
      payload: {
...
export async function runWorktreeSetupInBackground(
  dependencies: CreateSkynetWorktreeInBackgroundDependencies,
  options: {
    requestCwd: string;
    repoRoot: string;
    workspaceId: string;
