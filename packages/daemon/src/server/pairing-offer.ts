...
export async function generateLocalPairingOffer(args: {
  ...
  const relayEndpoint = args.relayEndpoint ?? "relay.skynet.sh:443";
  const relayPublicEndpoint = args.relayPublicEndpoint ?? relayEndpoint;
  const appBaseUrl = args.appBaseUrl ?? "https://app.skynet.sh";
  const serverId = getOrCreateServerId(args.skynetHome, { logger: args.logger });
  const daemonKeyPair = await loadOrCreateDaemonKeyPair(args.skynetHome, args.logger);
