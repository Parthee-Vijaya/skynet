...
import { archiveSkynetWorktree } from "./skynet-worktree-archive-service.js";
import {
  buildAgentSessionConfig,
  handleSkynetWorktreeArchiveRequest,
  handleSkynetWorktreeListRequest,
  resolveGitCreateBaseBranch,
  runWorktreeSetupInBackground,
  handleCreateSkynetWorktreeRequest,
...
import {
  createSkynetWorktree as createSkynetWorktreeService,
  type CreateSkynetWorktreeFn,
} from "./skynet-worktree-service.js";
...
function createSkynetWorktreeForTest(options: {
  skynetHome: string;
  events?: string[];
}): CreateSkynetWorktreeFn {
  ...
  return (input, serviceOptions) => {
    ...
    return createSkynetWorktreeService(input, {
      ...coreDeps,
...
describe("handleSkynetWorktreeListRequest", () => {
  test("lists worktrees through the workspace git service", async () => {
    const emitted: SessionOutboundMessage[] = [];
    const workspaceGitService = {
      listWorktrees: vi.fn().mockResolvedValue([
        {
          path: "/tmp/skynet-home/worktrees/repo/feature",
    ...
    await handleSkynetWorktreeListRequest(
      {
        emit: (message) => emitted.push(message),
        skynetHome: "/tmp/skynet-home",
        workspaceGitService: workspaceGitService as any,
      },
      {
        type: "skynet_worktree_list_request",
        cwd: "/tmp/repo",
    ...
    expect(emitted).toContainEqual({
      type: "skynet_worktree_list_response",
      payload: {
        worktrees: [
          {
            worktreePath: "/tmp/skynet-home/worktrees/repo/feature",
            createdAt: "2026-04-12T00:00:00.000Z",
            branchName: "feature",
            head: "abc123",
          },
...
function createGitRepo(options?: { skynetConfig?: Record<string, unknown> }) {
  ...
  if (options?.skynetConfig) {
    writeFileSync(path.join(repoDir, "skynet.json"), JSON.stringify(options.skynetConfig, null, 2));
  }
...
describe("runWorktreeSetupInBackground", () => {
  ...
  test("emits running then completed snapshots for no-setup workspaces without auto-starting scripts", async () => {
    ...
    const skynetHome = path.join(tempDir, ".skynet");
  ...
  test("archives the pending workspace and emits a failed snapshot when setup cannot start", async () => {
    ...
    writeFileSync(path.join(repoDir, "skynet.json"), "{ invalid json\n");
    execSync("git add skynet.json", { cwd: repoDir, stdio: "pipe" });
    ...
    expect(progressMessages[1]?.payload.error).toContain("Failed to parse skynet.json");
    expect(progressMessages[1]?.payload.detail.commands).toEqual([]);
    expect(snapshots.get(workspaceId)).toMatchObject({
      status: "failed",
      error: expect.stringContaining("Failed to parse skynet.json"),
    });
    ...
    const snapshots = new Map([
      [
        "ws-feature-a",
        {
          ...
          detail: {
            type: "worktree_setup",
            worktreePath: "/repo/.skynet/worktrees/feature-a",
...
describe("handleCreateSkynetWorktreeRequest", () => {
  const cleanupPaths: string[] = [];
  ...
  test("checks out the GitHub PR branch when a github_pr attachment is present", async () => {
    ...
    const response = emitted.find(
      (
        message,
      ): message is Extract<SessionOutboundMessage, { type: "create_skynet_worktree_response" }> =>
        message.type === "create_skynet_worktree_response",
    );
  ...
  test("buildAgentSessionConfig checks out the GitHub PR branch for agent worktrees", async () => {
    ...
    const result = await buildAgentSessionConfig(
      {
        skynetHome: path.join(tempDir, ".skynet"),
        sessionLogger: createLogger(),
        createSkynetWorktree: createSkynetWorktreeForTest({
          skynetHome: path.join(tempDir, ".skynet"),
          events,
        }),
  ...
  test("buildAgentSessionConfig invalidates GitHub cache after branch setup mutations", async () => {
    ...
    const createSkynetWorktree = vi.fn(async () => {
  ...
  test("createSkynetWorktreeForTest forwards the default branch resolver for branch-off intents", async () => {
    const { tempDir, repoDir } = createGitRepo();
    cleanupPaths.push(tempDir);
    const skynetHome = path.join(tempDir, ".skynet");
    const resolveDefaultBranch = vi.fn(async () => "main");

    const result = await createSkynetWorktreeForTest({ skynetHome })(
      {
        cwd: repoDir,
      ...
      await handleCreateSkynetWorktreeRequest(
        {
          ...
          sessionLogger: createLogger(),
          emit: (message) => emitted.push(message),
          createSkynetWorktree: createSkynetWorktreeForTest({ skynetHome, events }),
  ...
  test("creates the worktree before emitting the response", async () => {
    ...
    try {
      await handleCreateSkynetWorktreeRequest(
        {
          ...
          sessionLogger: createLogger(),
          emit: (message) => emitted.push(message),
          createSkynetWorktree: async (input) => {
            const result = await createSkynetWorktreeForTest({ skynetHome })(input);
      ...
      expect(emitted.map((message) => message.type).slice(0, 2)).toEqual([
        "create_skynet_worktree_response",
        "workspace_update",
      ]);
...
describe("archiveSkynetWorktree", () => {
  const cleanupPaths: string[] = [];
  ...
  test("runs agent close and terminal teardown concurrently and removes the worktree", async () => {
    ...
    const emitted: SessionOutboundMessage[] = [];
    const removedAgents = await archiveSkynetWorktree(
      {
        skynetHome,
        github: createGitHubServiceStub(),
        agentManager: {
  ...
  test("succeeds when git has forgotten about the worktree (no repoRoot)", async () => {
    ...
    const emitted: SessionOutboundMessage[] = [];
    await handleSkynetWorktreeArchiveRequest(
      {
        skynetHome,
        github: createGitHubServiceStub(),
        agentManager: {
      ...
      {
        type: "skynet_worktree_archive_request",
    ...
    const response = emitted.find(
      (
        message,
      ): message is Extract<SessionOutboundMessage, { type: "skynet_worktree_archive_response" }> =>
        message.type === "skynet_worktree_archive_response",
    );
