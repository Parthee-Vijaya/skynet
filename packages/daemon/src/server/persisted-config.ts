...
const DEFAULT_PERSISTED_CONFIG = PersistedConfigSchema.parse({
  ...
  daemon: {
    ...
    cors: {
      allowedOrigins: ["https://app.skynet.sh"],
    },
    relay: {
      enabled: true,
    },
...
function getConfigPath(skynetHome: string): string {
  return path.join(skynetHome, CONFIG_FILENAME);
}
...
export function loadPersistedConfig(skynetHome: string, logger?: LoggerLike): PersistedConfig {
  const log = getLogger(logger);
  const configPath = getConfigPath(skynetHome);
