...
import { createSkynetWorktree, type CreateSkynetWorktreeDeps } from "./skynet-worktree-service.js";
import { createWorktreeCoreDeps } from "./worktree-core.js";

describe("createSkynetWorktree", () => {
  ...
  test("creates a worktree and registers it in the source workspace project without git snapshot lookup", async () => {
    ...
    const result = await createSkynetWorktree(
      {
        cwd: repoDir,
  ...
  test("reuses an existing worktree and still upserts the workspace", async () => {
    ...
    const first = await createSkynetWorktree(
      {
        cwd: repoDir,
    ...
    const second = await createSkynetWorktree(
      {
        cwd: repoDir,
        worktreeSlug: "reuse-me",
        runSetup: false,
  ...
  test("does not mutate registries or broadcast when core worktree creation fails", async () => {
    const tempDir = mkdtempSync(path.join(tmpdir(), "skynet-worktree-service-"));
    cleanupPaths.push(tempDir);
    const deps = createDeps();
  ...
  test("keeps direct core worktree creation calls behind the service boundary", () => {
    ...
    expect(matches).toEqual(["skynet-worktree-service.test.ts", "skynet-worktree-service.ts"]);
  });
});

interface TestDeps extends CreateSkynetWorktreeDeps {
...
function createWorkspaceGitSnapshot(cwd: string): WorkspaceGitRuntimeSnapshot {
  ...
  return {
    ...
    git: {
      ...
      currentBranch,
      remoteUrl: null,
      isSkynetOwnedWorktree: repoRoot !== mainRepoRoot,
      isDirty: false,
      baseRef: "main",
      aheadBehind: null,
      aheadOfOrigin: null,
