...
describe("workspace aggregation", () => {
  test("workspace reconciliation reports archived workspaces to subscribed clients", async () => {
    const missingCwd = path.join(tmpdir(), `skynet-missing-workspace-${Date.now()}`);
  ...
  test("create skynet worktree request returns a registered workspace descriptor", async () => {
    const emitted: Array<{ type: string; payload: unknown }> = [];
    const tempDir = realpathSync(mkdtempSync(path.join(tmpdir(), "session-worktree-test-")));
    const repoDir = path.join(tempDir, "repo");
    const skynetHome = path.join(tempDir, "skynet-home");
    ...
    try {
      await session.handleCreateSkynetWorktreeRequest({
    ...
    const response = emitted.find((message) => message.type === "create_skynet_worktree_response") as
      | { type: "create_skynet_worktree_response"; payload: any }
      | undefined;
  ...
  test.skip("opening a new worktree reconciles older local workspaces into the remote project", async () => {
    ...
    const worktreeWorkspaceId = path.join(mainWorkspaceId, ".skynet", "worktrees", "feature-a");
    ...
    session.buildProjectPlacement = async (cwd: string) => ({
      ...
      checkout: {
        ...
        worktreeRoot: cwd,
        isSkynetOwnedWorktree: cwd !== mainWorkspaceId,
