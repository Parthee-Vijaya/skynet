...
function getServerIdPath(skynetHome: string): string {
  return path.join(skynetHome, SERVER_ID_FILENAME);
}
...
/**
 * Stable daemon identifier scoped to a given $SKYNET_HOME.
 *
 * - Persisted to `$SKYNET_HOME/server-id`
 * - Can be overridden via `SKYNET_SERVER_ID` (useful for tests)
 */
export function getOrCreateServerId(
  ...
  const serverIdPath = getServerIdPath(skynetHome);

  const envOverride =
    typeof env.SKYNET_SERVER_ID === "string" && env.SKYNET_SERVER_ID.trim().length > 0
      ? env.SKYNET_SERVER_ID.trim()
      : null;
  ...
  if (envOverride) {
    ...
    if (!existsSync(serverIdPath)) {
      try {
        ...
        log?.info({ serverId: envOverride }, "Persisted SKYNET_SERVER_ID override");
      } catch (error) {
        log?.warn({ error }, "Failed to persist SKYNET_SERVER_ID override");
      }
    }
    return envOverride;
  }
