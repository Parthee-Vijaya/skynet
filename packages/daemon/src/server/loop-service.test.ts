...
describe("LoopService", () => {
  ...
  beforeEach(() => {
    ...
    skynetHome = path.join(tmpDir, "skynet-home");
  ...
  test("runs fresh worker agents until verify-check passes", async () => {
    ...
    const service = new LoopService({ skynetHome, agentManager: manager, logger });
    ...
    expect(readFileSync(path.join(skynetHome, "loops", "loops.json"), "utf8")).toContain(loop.id);
  });
