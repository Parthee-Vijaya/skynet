...
describe("bootstrap provider availability", () => {
  ...
  test("loads a persisted Codex record without spawning a missing Codex binary", async () => {
    const { createSkynetDaemon } = await import("./bootstrap.js");
    const root = await mkdtemp(path.join(os.tmpdir(), "skynet-bootstrap-provider-"));
    tempRoots.push(root);
    const binDir = await mkdtemp(path.join(os.tmpdir(), "skynet-bootstrap-provider-bin-"));
    tempRoots.push(binDir);
    process.env.PATH = binDir;
    if (process.platform === "win32") {
      process.env.PATHEXT = ".CMD";
    }
    const skynetHome = path.join(root, ".skynet");
    const staticDir = path.join(root, "static");
    const agentStoragePath = path.join(skynetHome, "agents");
    ...
    const daemon = await createSkynetDaemon(config, pino({ level: "silent" }));
    try {
