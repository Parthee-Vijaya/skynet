...
import { createSkynetWorktree } from "./skynet-worktree-service.js";
...
export type SkynetOpenAIConfig = OpenAiSpeechProviderConfig;
export type SkynetLocalSpeechConfig = LocalSpeechProviderConfig;

export type SkynetSpeechConfig = {
  providers: RequestedSpeechProviders;
  local?: SkynetLocalSpeechConfig;
};
...
export type SkynetDaemonConfig = {
  listen: string;
  skynetHome: string;
...
export async function createSkynetDaemon(
  config: SkynetDaemonConfig,
  rootLogger: Logger,
): Promise<SkynetDaemon> {
  ...
  try {
    const serverId = getOrCreateServerId(config.skynetHome, { logger });
    const daemonKeyPair = await loadOrCreateDaemonKeyPair(config.skynetHome, logger);
    ...
    const allowedOrigins = new Set([
      ...
      // Packaged desktop renderers use the custom skynet:// protocol scheme.
      "skynet://app",
    ...
    const projectRegistry = new FileBackedProjectRegistry(
      path.join(config.skynetHome, "projects", "projects.json"),
      logger,
    );
    workspaceRegistry = new FileBackedWorkspaceRegistry(
      path.join(config.skynetHome, "projects", "workspaces.json"),
      logger,
    );
    const chatService = new FileBackedChatService({
      skynetHome: config.skynetHome,
      logger,
    });
    ...
    if (mcpEnabled) {
      ...
      const createAgentMcpTransport = async (callerAgentId?: string) => {
        const agentMcpServer = await createAgentMcpServer({
          ...
          createSkynetWorktree: async (input, serviceOptions) => {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        const onListening = () => {
          ...
          const logAndResolve = async () => {
            ...
            const relayEndpoint = config.relayEndpoint ?? "relay.skynet.sh:443";
            const relayPublicEndpoint = config.relayPublicEndpoint ?? relayEndpoint;
            const appBaseUrl = config.appBaseUrl ?? "https://app.skynet.sh";
            ...
            if (typeof process.send === "function" && process.env.SKYNET_SUPERVISED === "1") {
              process.send({
                type: "skynet:ready",
                listen:
