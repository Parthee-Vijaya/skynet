import { createSkynetDaemon } from "./bootstrap.js";
...
async function main() {
  ...
  let daemon: Awaited<ReturnType<typeof createSkynetDaemon>> | null = null;
  let shutdownPromise: Promise<number> | null = null;
  let exitHookInstalled = false;
  const supervised = process.env.SKYNET_SUPERVISED === "1" && typeof process.send === "function";
  let pidLockAcquired = false;

  try {
    skynetHome = resolveSkynetHome();
    const persistedConfig = loadPersistedConfig(skynetHome);
    logger = createRootLogger(persistedConfig, { skynetHome });
    config = loadConfig(skynetHome);
  } catch (err) {
    ...
    if (!shutdownPromise) {
      ...
      shutdownPromise = (async () => {
        ...
        try {
          ...
          await daemon.stop();
          if (pidLockAcquired) {
            await releasePidLock(skynetHome);
  ...
  const handleLifecycleIntent = (intent: DaemonLifecycleIntent) => {
    if (intent.type === "shutdown") {
      ...
      if (sendSupervisorLifecycleMessage({ type: "skynet:shutdown" })) {
        return;
      }
  ...
  try {
    if (!supervised) {
      await acquirePidLock(skynetHome, null);
      pidLockAcquired = true;
    }

    daemon = await createSkynetDaemon(
      {
        ...config,
  ...
  try {
    ...
    if (!supervised) {
      ...
      await updatePidLock(skynetHome, { listen });
    }
  } catch (err) {
    if (pidLockAcquired) {
      await releasePidLock(skynetHome);
