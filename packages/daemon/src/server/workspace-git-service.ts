...
import { listSkynetWorktrees, type SkynetWorktreeInfo } from "../utils/worktree.js";
...
export type WorkspaceGitWorktreeInfo = SkynetWorktreeInfo;
...
interface WorkspaceGitServiceDependencies {
  ...
  listSkynetWorktrees: typeof listSkynetWorktrees;
...
export class WorkspaceGitServiceImpl implements WorkspaceGitService {
  ...
  constructor(options: WorkspaceGitServiceOptions) {
    ...
    this.skynetHome = options.skynetHome;
    this.deps = {
      ...
      listSkynetWorktrees: options.deps?.listSkynetWorktrees ?? listSkynetWorktrees,
  ...
  getCheckoutDiff(
    ...
    return this.readAuxiliaryCache(this.checkoutDiffCache, key, readOptions, () =>
      this.deps.getCheckoutDiff(normalizedCwd, normalizedOptions, { skynetHome: this.skynetHome }),
    );
  }
  ...
  listStashes(
    ...
    const normalizedCwd = normalizeWorkspaceId(cwd);
    const skynetOnly = options?.skynetOnly !== false;
    const key = JSON.stringify(["stashes", normalizedCwd, skynetOnly]);
    return this.readAuxiliaryCache(this.stashListCache, key, readOptions, async () => {
      ...
      return parseWorkspaceGitStashList(stdout, { skynetOnly });
    });
  }
  ...
  async resolveRepoRoot(cwd: string, options?: WorkspaceGitReadOptions): Promise<string> {
    ...
    return snapshot.git.isSkynetOwnedWorktree
...
async function loadWorkspaceGitRuntimeSnapshot(
  ...
  return {
    ...
    git: {
      ...
      mainRepoRoot: checkoutStatus.isSkynetOwnedWorktree ? checkoutStatus.mainRepoRoot : null,
      currentBranch: checkoutStatus.currentBranch,
      remoteUrl: checkoutStatus.remoteUrl,
      isSkynetOwnedWorktree: checkoutStatus.isSkynetOwnedWorktree,
...
function parseWorkspaceGitStashList(
  ...
  for (const line of lines) {
    ...
    const prefix = "skynet-auto-stash:";
    const prefixIdx = subject.indexOf(prefix);
    const isSkynet = prefixIdx >= 0;
    const branch = isSkynet ? subject.slice(prefixIdx + prefix.length).trim() || null : null;

    if (options.skynetOnly && !isSkynet) {
      continue;
    }

    entries.push({ index, message: subject, branch, isSkynet });
  }
