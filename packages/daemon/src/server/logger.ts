...
import { resolveSkynetHome } from "./skynet-home.js";
...
function resolveFilePath(skynetHome: string, configuredPath: string | undefined): string {
  const fallback = path.join(skynetHome, DEFAULT_DAEMON_LOG_FILENAME);
  if (!configuredPath) {
    return fallback;
  }
  ...
  return path.resolve(skynetHome, configuredPath);
}
...
function resolveConfiguredSkynetHome(options: ResolveLogConfigOptions | undefined): string {
  if (options?.skynetHome) {
    return options.skynetHome;
  }
  return resolveSkynetHome(options?.env ?? process.env);
}
...
export function resolveLogConfig(
  ...
  const skynetHome = resolveConfiguredSkynetHome(options);
  const persistedLog = persistedConfig?.log;

  const envGlobalLevel = parseLogLevel(env.SKYNET_LOG);
  const persistedGlobalLevel = persistedLog?.level;
  const consoleLevel: LogLevel =
    parseLogLevel(env.SKYNET_LOG_CONSOLE_LEVEL) ??
    envGlobalLevel ??
  ...
  const fileLevel: LogLevel =
    parseLogLevel(env.SKYNET_LOG_FILE_LEVEL) ??
    envGlobalLevel ??
  ...
  const consoleFormat: LogFormat =
    parseLogFormat(env.SKYNET_LOG_FORMAT) ??
    persistedLog?.console?.format ??
    persistedLog?.format ??
    DEFAULT_CONSOLE_FORMAT;

  const filePath = resolveFilePath(skynetHome, env.SKYNET_LOG_FILE_PATH ?? persistedLog?.file?.path);

  const rotateMaxSize =
    env.SKYNET_LOG_FILE_ROTATE_SIZE?.trim() ||
    persistedLog?.file?.rotate?.maxSize ||
    DEFAULT_FILE_ROTATE_SIZE;

  const rotateMaxFiles =
    parsePositiveInteger(env.SKYNET_LOG_FILE_ROTATE_COUNT) ??
