  ...
  test("custom agents defined in opencode.json appear in available modes", async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        agent: {
          "skynet-test-custom": {
            description: "Custom agent defined for Skynet integration test",
            mode: "primary",
          },
        },
      }),
    ...
    const custom = modes.find((mode) => mode.id === "skynet-test-custom");
    expect(custom).toBeDefined();
    expect(custom!.label).toBe("Skynet-test-custom");
    expect(custom!.description).toBe("Custom agent defined for Skynet integration test");
    ...
    const parts = __openCodeInternals.buildOpenCodePromptParts([
      {
        ...
        url: "https://github.com/getskynet/skynet/issues/55",
        body: "Issue body",
      },
    ]);
