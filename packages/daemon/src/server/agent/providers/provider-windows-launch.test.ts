...
const JSON_ARG = '{"mcpServers":{"skynet":{"type":"http","url":"http://127.0.0.1:6767/mcp"}}}';
const tempDirs: string[] = [];

function makeFixture(binaryName: string, expectedArgs: string[]) {
  const root = mkdtempSync(path.join(tmpdir(), `skynet ${binaryName} launch `));
  tempDirs.push(root);

  const fakeDaemonNode = path.join(root, "Fake Skynet.exe");
...
const expected = JSON.parse(process.env.SKYNET_EXPECTED_ARGV_JSON);
...
async function runProviderFixture(params: {
  ...
  const child = spawnProcess(params.command, params.args, {
    env: {
      ...process.env,
      SKYNET_EXPECTED_ARGV_JSON: JSON.stringify(params.expectedArgs),
    },
