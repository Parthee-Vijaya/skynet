...
describe("codex tool-call mapper", () => {
  ...
  it("normalizes codex skynet speak mcp calls and extracts spoken text", () => {
    const item = mapCodexToolCallFromThreadItem({
      type: "mcpToolCall",
  ...
  it("normalizes codex skynet_voice.speak mcp calls and extracts spoken text", () => {
    const item = mapCodexToolCallFromThreadItem({
      type: "mcpToolCall",
      ...
      server: "skynet_voice",
      tool: "speak",
      arguments: { text: "Voice response from Codex via skynet_voice." },
      result: { ok: true },
    });
    ...
    expect(item?.detail).toEqual({
      type: "unknown",
      input: "Voice response from Codex via skynet_voice.",
      output: null,
    });
  });

  it("normalizes codex skynet speak rollout names and extracts spoken text", () => {
    const item = expectMapped(
      mapCodexRolloutToolCall({
