...
describe("Claude agent env", () => {
  test("forwards launch-context env through Claude process env", async () => {
    ...
    const launchContext: AgentLaunchContext = {
      env: {
        SKYNET_AGENT_ID: "00000000-0000-4000-8000-000000000201",
        SKYNET_TEST_FLAG: "launch-value",
      },
    };
    ...
    try {
      ...
      expect(capturedEnv?.SKYNET_AGENT_ID).toBe(launchContext.env?.SKYNET_AGENT_ID);
      expect(capturedEnv?.SKYNET_TEST_FLAG).toBe(launchContext.env?.SKYNET_TEST_FLAG);
    } finally {
      await session.close();
    }
  });
  ...
  test("forwards launch-context env through Claude resume env", async () => {
    ...
    const launchContext: AgentLaunchContext = {
      env: {
        SKYNET_AGENT_ID: "00000000-0000-4000-8000-000000000202",
        SKYNET_TEST_FLAG: "resume-launch-value",
      },
    };
    const queryFactory = vi.fn(
      ({ options }: { options: { env?: Record<string, string | undefined> } }) => {
