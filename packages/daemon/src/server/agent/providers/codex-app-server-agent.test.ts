...
describe("Codex app-server provider", () => {
  ...
  test("builds app-server env from launch-context env overrides", () => {
    const launchContext: AgentLaunchContext = {
      env: {
        SKYNET_AGENT_ID: "00000000-0000-4000-8000-000000000301",
        SKYNET_TEST_FLAG: "codex-launch-value",
      },
    };
    const env = __codexAppServerInternals.buildCodexAppServerEnv(
      {
        env: {
          SKYNET_AGENT_ID: "runtime-value",
          SKYNET_TEST_FLAG: "runtime-test-value",
        },
      },
      launchContext.env,
    );

    expect(env.SKYNET_AGENT_ID).toBe(launchContext.env?.SKYNET_AGENT_ID);
    expect(env.SKYNET_TEST_FLAG).toBe(launchContext.env?.SKYNET_TEST_FLAG);
  });
