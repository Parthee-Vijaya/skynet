...
describe("ClaudeAgentSession redesign invariants", () => {
  ...
  test("logs redacted query summary and never leaks sentinel secrets", async () => {
    const envSecret = "SKYNET_ENV_SENTINEL_SECRET";
    const runtimeSecret = "SKYNET_RUNTIME_SENTINEL_SECRET";
    const systemSecret = "SKYNET_SYSTEM_PROMPT_SENTINEL_SECRET";
    const previousEnv = process.env.SKYNET_TEST_SENTINEL_SECRET;
    process.env.SKYNET_TEST_SENTINEL_SECRET = envSecret;
    ...
    const client = new ClaudeAgentClient({
      ...
      runtimeSettings: {
        env: {
          SKYNET_RUNTIME_SENTINEL_SECRET: runtimeSecret,
        },
      },
    });
    ...
    try {
      ...
      if (previousEnv === undefined) {
        delete process.env.SKYNET_TEST_SENTINEL_SECRET;
      } else {
        process.env.SKYNET_TEST_SENTINEL_SECRET = previousEnv;
      }
    }
  });
