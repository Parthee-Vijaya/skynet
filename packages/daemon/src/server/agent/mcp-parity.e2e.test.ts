...
import { createTestSkynetDaemon, type TestSkynetDaemon } from "../test-utils/skynet-daemon.js";
...
describe("MCP parity end-to-end", () => {
  let tempRoot: string;
  let daemonHandle: TestSkynetDaemon;
  ...
  beforeAll(async () => {
    ...
    daemonHandle = await createTestSkynetDaemon();
  ...
  describe("Suite A: Core Fixes", () => {
    ...
    test("create_agent with callerAgentId sets skynet.parent-agent-id label", async () => {
      ...
      try {
        ...
        expect(snapshot?.labels).toMatchObject({
          "skynet.parent-agent-id": parentAgentId,
        });
      } finally {
    ...
    test("agentManager.createAgent injects skynet MCP using the daemon listen target", async () => {
      let agentId: string | null = null;
      try {
