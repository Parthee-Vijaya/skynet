...
import { getSkynetWorktreesRoot, type WorktreeConfig } from "../../utils/worktree.js";
import {
...
export interface AgentMcpServerOptions {
  ...
  archiveWorkspaceRecord?: ArchiveSkynetWorktreeDependencies["archiveWorkspaceRecord"];
  emitWorkspaceUpdatesForCwds?: ArchiveSkynetWorktreeDependencies["emitWorkspaceUpdatesForCwds"];
  emitSessionMessage?: ArchiveSkynetWorktreeDependencies["emit"];
  createSkynetWorktree?: CreateSkynetWorktreeFn;
  skynetHome?: string;
  /**
...
export async function createAgentMcpServer(options: AgentMcpServerOptions): Promise<McpServer> {
  ...
  server.registerTool(
    ...
    async (args: unknown) => {
      ...
      if (callerAgentId) {
        ...
        if (worktreeName) {
          ...
          const createdWorktree = await createMcpWorktree({
            ...
            createSkynetWorktree: options.createSkynetWorktree,
      ...
      const mergedLabels = {
        ...(callerAgentId ? { "skynet.parent-agent-id": callerAgentId } : {}),
  ...
  server.registerTool(
    "list_worktrees",
    {
      ...
      description: "List Skynet-managed git worktrees for a repository.",
      inputSchema: {
        cwd: z
          .string()
          .optional()
  ...
  server.registerTool(
    "create_worktree",
    {
      ...
      description: "Create a Skynet-managed git worktree.",
      inputSchema: {
        cwd: z
          .string()
          .optional()
  ...
  server.registerTool(
    "archive_worktree",
    {
      ...
      description: "Delete a Skynet-managed git worktree.",
      inputSchema: {
        cwd: z
          .string()
          .optional()
    ...
    async ({ cwd, worktreePath, worktreeSlug }) => {
      ...
      const targetPath =
        worktreePath ??
        join(await getSkynetWorktreesRoot(repoRoot, options.skynetHome), worktreeSlug!);
...
interface CreateMcpWorktreeOptions {
  ...
  createSkynetWorktree: CreateSkynetWorktreeFn | undefined;
...
async function createMcpWorktree(
  ...
  try {
    if (!options.createSkynetWorktree) {
      throw new Error("Skynet worktree service is not configured");
    }
    const result = await options.createSkynetWorktree(options.input, {
