...
export function isSkynetToolName(name: string): boolean {
  const normalized = normalizeToolName(name);
  ...
  if (normalized.includes("__")) {
    ...
    return (
      segments.length >= 3 &&
      segments[0] === "mcp" &&
      (segments[1] === "skynet" || segments[1]!.startsWith("skynet_"))
    );
  }
  if (normalized.includes(".")) {
    const firstSegment = normalized.split(".")[0]!;
    return firstSegment === "skynet" || firstSegment.startsWith("skynet_");
  }
  return false;
}

export function getSkynetToolLeafName(name: string): string | null {
  const normalized = normalizeToolName(name);
  if (normalized.includes("__")) {
    const segments = normalized.split("__").filter((s) => s.length > 0);
    if (
  ...
  if (normalized.includes(".")) {
    const firstSegment = normalized.split(".")[0]!;
    if (firstSegment === "skynet" || firstSegment.startsWith("skynet_")) {
