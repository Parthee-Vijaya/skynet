...
async function canRenameBranch(
  ...
  if (!snapshot.git.isGit || !snapshot.git.isSkynetOwnedWorktree) {
    return false;
  }
...
export async function determineAgentMetadataNeeds(
  options: Pick<
    ...
    "initialPrompt" | "explicitTitle" | "cwd" | "skynetHome" | "deps"
  >,
...
export async function generateAndApplyAgentMetadata(
  ...
  if (needs.needsBranch && typeof result.branch === "string") {
    ...
    if (!snapshot.git.isGit || !snapshot.git.isSkynetOwnedWorktree || !snapshot.git.currentBranch) {
      return;
    }
