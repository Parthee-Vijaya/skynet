...
import { createSkynetDaemon, type SkynetDaemonConfig } from "../bootstrap.js";
...
describe("agent MCP end-to-end (offline)", () => {
  test("create_agent runs initial prompt and affects filesystem", async () => {
    const skynetHome = await mkdtemp(path.join(os.tmpdir(), "skynet-home-"));
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "skynet-static-"));
    const agentCwd = await mkdtemp(path.join(os.tmpdir(), "skynet-agent-cwd-"));
    const port = await getAvailablePort();

    const daemonConfig: SkynetDaemonConfig = {
    ...
    const daemon = await createSkynetDaemon(daemonConfig, pino({ level: "silent" }));
    await daemon.start();
  ...
  test("create_agent auto-injects skynet MCP by default and can be disabled", async () => {
    const skynetHome = await mkdtemp(path.join(os.tmpdir(), "skynet-home-"));
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "skynet-static-"));
    const agentCwd = await mkdtemp(path.join(os.tmpdir(), "skynet-agent-cwd-"));
    const port = await getAvailablePort();
    ...
    const disabledSkynetHome = await mkdtemp(path.join(os.tmpdir(), "skynet-home-disabled-"));
    const disabledStaticDir = await mkdtemp(path.join(os.tmpdir(), "skynet-static-disabled-"));
    const disabledAgentCwd = await mkdtemp(path.join(os.tmpdir(), "skynet-agent-cwd-disabled-"));
    const disabledPort = await getAvailablePort();
    const disabledDaemonConfig: SkynetDaemonConfig = {
      listen: `127.0.0.1:${disabledPort}`,
      skynetHome: disabledSkynetHome,
      corsAllowedOrigins: [],
      hostnames: true,
      mcpEnabled: true,
      ...
      mcpDebug: false,
      agentClients: createTestAgentClients(),
      agentStoragePath: path.join(disabledSkynetHome, "agents"),
    };
    const disabledDaemon = await createSkynetDaemon(disabledDaemonConfig, pino({ level: "silent" }));
    ...
    try {
      ...
      expect(disabledAgent?.config.mcpServers?.skynet).toBeUndefined();
    } finally {
      if (agentId) {
        await client.callTool({ name: "kill_agent", args: { agentId } });
      }
      ...
      await rm(disabledSkynetHome, { recursive: true, force: true });
  ...
  test("create_agent with worktree is async and boots terminals only after setup success", async () => {
    const skynetHome = await mkdtemp(path.join(os.tmpdir(), "skynet-home-"));
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "skynet-static-"));
    const repoRoot = await mkdtemp(path.join(os.tmpdir(), "skynet-worktree-repo-"));
    const port = await getAvailablePort();
