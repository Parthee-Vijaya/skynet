...
describe("shared tool-call display mapping", () => {
  ...
  it("builds display model for worktree setup detail", () => {
    const display = buildToolCallDisplayModel({
      name: "skynet_worktree_setup",
      status: "running",
      error: null,
      detail: {
        type: "worktree_setup",
        worktreePath: "/tmp/repo/.skynet/worktrees/repo/branch",
        ...
        commands: [
          {
            index: 1,
            command: "npm install",
            cwd: "/tmp/repo/.skynet/worktrees/repo/branch",
            log: "==> [1/1] Running: npm install\n",
            status: "running",
            exitCode: null,
          },
  ...
  it("humanizes Skynet MCP tool names (Claude Code format)", () => {
    const display = buildToolCallDisplayModel({
      name: "mcp__skynet__create_agent",
      status: "running",
      error: null,
  ...
  it("humanizes Skynet MCP tool names (Codex format)", () => {
    const display = buildToolCallDisplayModel({
      name: "skynet.create_agent",
      status: "running",
      error: null,
      detail: { type: "unknown", input: null, output: null },
    });
  ...
  it("humanizes list_agents Skynet tool", () => {
    const display = buildToolCallDisplayModel({
      name: "mcp__skynet__list_agents",
      status: "running",
      error: null,
      detail: { type: "unknown", input: null, output: null },
    });
