    ...
    data: {
      repository: {
        pullRequest: {
          ...
          headRepository: {
            sshUrl: "git@github.com:therainisme/skynet.git",
            url: "https://github.com/therainisme/skynet",
          },
        },
      },
    },
...
describe("GitHubService", () => {
  ...
  it("loads pull request checkout target details through GraphQL", async () => {
    ...
    await expect(
      ...
      headRepositorySshUrl: "git@github.com:therainisme/skynet.git",
      headRepositoryUrl: "https://github.com/therainisme/skynet",
      isCrossRepository: true,
    });
    ...
    expect(runner.calls[1]?.args).toContain("owner=getskynet");
    expect(runner.calls[1]?.args).toContain("name=skynet");
  ...
  it("resolves GitHub repos from a WorkspaceGitService-owned remote URL", async () => {
    const workspaceGitService = {
      resolveRepoRemoteUrl: async (cwd: string) => {
        ...
        return "git@github.com:getskynet/skynet.git";
      },
    };
