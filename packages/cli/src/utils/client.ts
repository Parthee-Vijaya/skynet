...
import { loadConfig, resolveSkynetHome, DaemonClient } from "@skynet/daemon";
...
function readPidSocketTarget(skynetHome: string): string | null {
  const pidPath = path.join(skynetHome, PID_FILENAME);
...
function resolveConfiguredIpcDaemonHost(env: NodeJS.ProcessEnv, skynetHome: string): string | null {
  const directEnvHost = normalizeDaemonHost(env.SKYNET_LISTEN ?? "");
  if (isIpcDaemonHost(directEnvHost)) {
    return directEnvHost;
  }

  const pidHost = normalizeDaemonHost(readPidSocketTarget(skynetHome) ?? "");
  if (isIpcDaemonHost(pidHost)) {
    return pidHost;
  }

  const config = loadConfig(skynetHome, { env });
  const configuredHost = normalizeDaemonHost(config.listen);
  return isIpcDaemonHost(configuredHost) ? configuredHost : null;
}

function resolveConfiguredTcpDaemonHost(env: NodeJS.ProcessEnv, skynetHome: string): string | null {
  const configuredHost = normalizeDaemonHost(loadConfig(skynetHome, { env }).listen);
...
export function resolveDefaultDaemonHosts(env: NodeJS.ProcessEnv = process.env): string[] {
  const skynetHome = resolveSkynetHome(env);
  const candidates: string[] = [];
  const configuredIpcHost = resolveConfiguredIpcDaemonHost(env, skynetHome);
  if (configuredIpcHost) {
    candidates.push(configuredIpcHost);
  }
  const configuredTcpHost = resolveConfiguredTcpDaemonHost(env, skynetHome);
...
function resolveDaemonHostCandidates(options?: ConnectOptions): string[] {
  const explicitHost = options?.host ?? process.env.SKYNET_HOST;
  if (explicitHost) {
    return [explicitHost];
  }
...
export async function connectToDaemon(options?: ConnectOptions): Promise<DaemonClient> {
  ...
  throw new Error(`Unable to connect to Skynet daemon via ${hosts.join(", ")}`);
}
