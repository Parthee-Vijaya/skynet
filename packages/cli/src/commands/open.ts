...
function findDesktopApp(): string | null {
  if (process.platform === "darwin") {
    const candidates = [
      ...
      path.join(homedir(), "Applications", "Skynet.app"),
    ];
  ...
  if (process.platform === "linux") {
    const candidates = [
      "/usr/bin/Skynet",
      "/opt/Skynet/Skynet",
      path.join(homedir(), "Applications", "Skynet.AppImage"),
    ];
  ...
  if (process.platform === "win32") {
    ...
    const candidate = path.join(localAppData, "Programs", "Skynet", "Skynet.exe");
...
export async function openDesktopWithProject(projectPath: string): Promise<void> {
  try {
    if (process.env.SKYNET_DESKTOP_CLI === "1") {
      throw new Error(
    ...
    if (!desktopApp) {
      throw new Error(
        "Skynet desktop app not found. Install it from https://github.com/getskynet/skynet/releases",
      );
    }
