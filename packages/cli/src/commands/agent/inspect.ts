...
import type { AgentSnapshotPayload } from "@skynet/daemon";
...
function toInspectData(snapshot: AgentSnapshotPayload): AgentInspect {
  ...
  const worktree = snapshot.labels?.["skynet.worktree"] ?? null;
  const parentAgentId = snapshot.labels?.["skynet.parent-agent-id"] ?? null;
...
export async function runInspectCommand(
  ...
  if (!agentIdArg || agentIdArg.trim().length === 0) {
    const error: CommandError = {
      ...
      details: "Usage: skynet agent inspect <id>",
    };
    throw error;
  }
  ...
  try {
    ...
    const error: CommandError = {
      ...
      details: "Start the daemon with: skynet daemon start",
    };
    throw error;
  }
  ...
  try {
    ...
    if (!fetchResult) {
      const error: CommandError = {
        ...
        details: 'Use "skynet ls" to list available agents',
      };
      throw error;
    }
