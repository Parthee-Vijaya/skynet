...
import type { DaemonClient, AgentStreamMessage, AgentTimelineItem } from "@skynet/daemon";
import { curateAgentActivity } from "@skynet/daemon";
...
export async function runLogsCommand(
  ...
  if (!id) {
    ...
    console.error("Usage: skynet agent logs <id>");
    process.exit(1);
  }
  ...
  try {
    ...
    console.error("Start the daemon with: skynet daemon start");
    process.exit(1);
  }
  ...
  try {
    ...
    if (!fetchResult) {
      ...
      console.error("Use `skynet ls` to list available agents");
      await client.close();
      process.exit(1);
    }
