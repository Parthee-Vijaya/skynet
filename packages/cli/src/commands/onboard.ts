...
function savePersistedConfig(skynetHome: string, config: OnboardPersistedConfig): void {
  const configPath = path.join(skynetHome, "config.json");
...
function printNextSteps(pairingUrl: string | null, skynetHome: string, richUi: boolean): void {
  const daemonLogPath = path.join(skynetHome, "daemon.log");
  const nextStepsLines = [
    pairingUrl
      ? "1. Open Skynet and scan the QR code above, or paste the pairing link."
      : "1. Open Skynet and connect to your daemon.",
    "2. Web app: https://app.skynet.sh",
    "3. Desktop app: https://github.com/getskynet/skynet/releases/latest",
    "4. Docs: https://skynet.sh/docs",
    '5. Example: skynet run --output-schema schema.json "extract fields"',
  ];
  const quickReferenceLines = [
    "1. skynet --help",
    "2. skynet ls",
    '3. skynet run "your prompt"',
    "4. skynet status",
...
export function onboardCommand(): Command {
  return new Command("onboard")
    ...
    .option("--home <path>", "Skynet home directory (default: ~/.skynet)")
...
export async function runOnboard(options: OnboardOptions): Promise<void> {
  ...
  const skynetHome = resolveLocalSkynetHome(options.home);
  if (richUi) {
    renderNote(skynetHome, "Skynet home");
  }

  let persisted = loadPersistedConfig(skynetHome) as OnboardPersistedConfig;
  ...
  savePersistedConfig(skynetHome, persisted);

  const config = loadConfig(skynetHome, { cli: toCliOverrides(options) });
  ...
  if (config.relayEnabled === false) {
    ...
    printNextSteps(null, skynetHome, richUi);
    if (richUi) {
      outro("Skynet daemon is running.");
    }
    return;
  }
  ...
  printNextSteps(pairing.url, skynetHome, richUi);
  if (richUi) {
    outro("Skynet is ready!");
  }
}
