...
export function resolveSkynetHomePath(): string {
  return process.env.SKYNET_HOME ?? join(homedir(), ".skynet");
}

export function resolveSkynetWorktreesDir(): string {
  return join(resolveSkynetHomePath(), "worktrees");
}

function isAgentInManagedWorktree(agentCwd: string): boolean {
  const worktreesDir = resolveSkynetWorktreesDir();
...
export async function runLsCommand(
  ...
  try {
    ...
    const response = await client.getSkynetWorktreeList({});
