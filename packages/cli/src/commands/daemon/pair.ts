...
import { generateLocalPairingOffer, loadConfig, resolveSkynetHome } from "@skynet/daemon";
...
export async function runPairCommand(options: PairOptions): Promise<void> {
  if (options.home) {
    process.env.SKYNET_HOME = options.home;
  }

  const skynetHome = resolveSkynetHome();
  const config = loadConfig(skynetHome);
