...
try {
  {
    ...
    const created = await ctx.skynet(
    ...
    const listed = await ctx.skynet(["schedule", "ls", "--json"]);
    ...
    const inspected = await ctx.skynet(["schedule", "inspect", createdJson.id, "--json"]);
...

  {
    ...
    const listed = await ctx.skynet(["loop", "ls", "--json"]);
    assert.strictEqual(listed.exitCode, 0, listed.stderr);
    const listedJson = JSON.parse(listed.stdout);
    assert(Array.isArray(listedJson), listed.stdout);
    assert(
    ...
    for (let attempt = 0; attempt < 40; attempt += 1) {
      const inspect = await ctx.skynet(["loop", "inspect", runJson.id, "--json"]);
    ...
    const logs = await ctx.skynet(["loop", "logs", runJson.id], { timeout: 15000 });
    assert.strictEqual(logs.exitCode, 0, logs.stderr);
    assert(logs.stdout.includes("verify-check"), logs.stdout);

    const stopped = await ctx.skynet(["loop", "stop", runJson.id, "--json"]);
