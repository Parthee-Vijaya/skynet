...
/**
 * Test runner for Skynet CLI E2E tests
 *
...
async function writeJsonSummary({
  ...
  await writeFile(
    ...
    JSON.stringify(
      {
        suite: "cli-local",
        command: "npm run test:local --workspace=@skynet/cli",
        counts: {
          passed,
          failed,
          skipped: 0,
...
console.log("🧪 Skynet CLI E2E Test Runner\n");
console.log("=".repeat(50));
...
await runCommand("Building relay", "npm run build --workspace=@skynet/relay");
await runCommand("Building server", "npm run build --workspace=@skynet/daemon");
await runCommand("Building CLI", "npm run build --workspace=@skynet/cli");
...
for (const testFile of testFiles) {
  ...
  try {
    const result =
      await $`SKYNET_LOCAL_SPEECH_AUTO_DOWNLOAD=${testEnvDefaults.SKYNET_LOCAL_SPEECH_AUTO_DOWNLOAD} SKYNET_DICTATION_ENABLED=${testEnvDefaults.SKYNET_DICTATION_ENABLED} SKYNET_VOICE_MODE_ENABLED=${testEnvDefaults.SKYNET_VOICE_MODE_ENABLED} npx tsx ${testPath}`.nothrow();
    if (result.exitCode === 0) {
      console.log(`\n✅ ${testName} PASSED`);
      passed++;
    } else {
