...
async function readPidLockPid(skynetHome: string): Promise<number | null> {
...
const skynetHome = await mkdtemp(join(tmpdir(), "skynet-restart-unsupervised-"));
...
try {
  ...
  const lockPid = await readPidLockPid(skynetHome);
  ...
  await waitFor(
    async () => (await readPidLockPid(skynetHome)) === null,
    15000,
