 ...
 *
 * Tests daemon commands with an isolated SKYNET_HOME.
 *
 * Tests:
...
function daemonCommand(args: string[]) {
  return runLocalSkynet(["daemon", ...args], { SKYNET_HOME: skynetHome });
}
...
try {
  // Test 1: daemon --help shows subcommands
  {
    ...
    const result = await runLocalSkynet(["daemon", "--help"]);
  ...
  // Test 5: daemon status --json outputs valid JSON
  {
    ...
    assert.strictEqual(status.home, skynetHome, "json status should reflect the isolated home");
    assert.strictEqual(
      status.hostname,
      null,
