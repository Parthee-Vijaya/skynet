...
try {
  // Test 1: wait --help shows options
  {
    ...
    const result = await $`npx skynet wait --help`.nothrow();
  ...
  // Test 2: wait requires id argument
  {
    ...
    const result =
      await $`SKYNET_HOST=localhost:${port} SKYNET_HOME=${skynetHome} npx skynet wait`.nothrow();
  ...
  // Test 3: wait handles daemon not running
  {
    ...
    const result =
      await $`SKYNET_HOST=localhost:${port} SKYNET_HOME=${skynetHome} npx skynet wait abc123`.nothrow();
  ...
  // Test 4: wait --timeout flag is accepted
  {
    ...
    const result =
      await $`SKYNET_HOME=${skynetHome} npx skynet wait --timeout 30 --host localhost:${port} abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: wait --host flag is accepted
  {
    ...
    const result =
      await $`SKYNET_HOME=${skynetHome} npx skynet wait --host localhost:${port} abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: -q (quiet) flag is accepted with wait
  {
    ...
    const result =
      await $`SKYNET_HOST=localhost:${port} SKYNET_HOME=${skynetHome} npx skynet -q wait abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: --json flag is accepted with wait
  {
    ...
    const result =
      await $`SKYNET_HOST=localhost:${port} SKYNET_HOME=${skynetHome} npx skynet wait abc123 --json`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8: --format yaml flag is accepted with wait
  {
    ...
    const result =
      await $`SKYNET_HOST=localhost:${port} SKYNET_HOME=${skynetHome} npx skynet --format yaml wait abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 9: skynet --help shows wait command
  {
    console.log("Test 9: skynet --help shows wait command");
    ...
    console.log("skynet --help shows wait command\n");
  }
