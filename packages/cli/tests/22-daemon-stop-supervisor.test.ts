...
/**
 * Regression: `skynet daemon stop` must stop supervised dev daemons
...
async function readPidLockState(skynetHome: string): Promise<PidLockState> {
  const pidPath = join(skynetHome, "skynet.pid");
...
async function readDaemonStatus(skynetHome: string): Promise<DaemonStatus> {
  const result =
    await $`SKYNET_HOME=${skynetHome} SKYNET_LOCAL_SPEECH_AUTO_DOWNLOAD=${testEnv.SKYNET_LOCAL_SPEECH_AUTO_DOWNLOAD} SKYNET_DICTATION_ENABLED=${testEnv.SKYNET_DICTATION_ENABLED} SKYNET_VOICE_MODE_ENABLED=${testEnv.SKYNET_VOICE_MODE_ENABLED} npx skynet daemon status --home ${skynetHome} --json`.nothrow();
...
const skynetHome = await mkdtemp(join(tmpdir(), "skynet-stop-supervisor-"));
...
try {
  console.log("Test 1: start supervisor-entrypoint in dev mode with isolated SKYNET_HOME");
  ...
  supervisorProcess = spawn(
    ...
    ["--import", "tsx", "../server/scripts/supervisor-entrypoint.ts", "--dev"],
    {
      ...
      env: {
        ...
        SKYNET_LISTEN: `127.0.0.1:${port}`,
        SKYNET_RELAY_ENABLED: "false",
        CI: "true",
      },
  ...
  await waitFor(
    async () => {
      const status = await readDaemonStatus(skynetHome);
      return (
  ...
  const statusBeforeStop = await readDaemonStatus(skynetHome);
  ...
  const pidLockBeforeStop = await readPidLockState(skynetHome);
  ...
  console.log("Test 2: `skynet daemon stop` should stop without respawn");
  const stopResult =
    await $`SKYNET_HOME=${skynetHome} SKYNET_LOCAL_SPEECH_AUTO_DOWNLOAD=${testEnv.SKYNET_LOCAL_SPEECH_AUTO_DOWNLOAD} SKYNET_DICTATION_ENABLED=${testEnv.SKYNET_DICTATION_ENABLED} SKYNET_VOICE_MODE_ENABLED=${testEnv.SKYNET_VOICE_MODE_ENABLED} npx skynet daemon stop --home ${skynetHome} --json`.nothrow();
  ...
  const pidAfterStop = await readPidLockState(skynetHome);
  ...
  const statusAfterStop = await readDaemonStatus(skynetHome);
  assert.strictEqual(
  ...
  await $`SKYNET_HOME=${skynetHome} SKYNET_LOCAL_SPEECH_AUTO_DOWNLOAD=${testEnv.SKYNET_LOCAL_SPEECH_AUTO_DOWNLOAD} SKYNET_DICTATION_ENABLED=${testEnv.SKYNET_DICTATION_ENABLED} SKYNET_VOICE_MODE_ENABLED=${testEnv.SKYNET_VOICE_MODE_ENABLED} npx skynet daemon stop --home ${skynetHome} --force`.nothrow();
