...
try {
  // Test 1: permit --help shows subcommands
  {
    ...
    const result = await $`npx skynet permit --help`.nothrow();
  ...
  // Test 2: permit ls --help shows options
  {
    ...
    const result = await $`npx skynet permit ls --help`.nothrow();
  ...
  // Test 3: permit ls returns error when no daemon running
  {
    ...
    const result =
      await $`SKYNET_HOST=localhost:${port} SKYNET_HOME=${skynetHome} npx skynet permit ls`.nothrow();
  ...
  // Test 4: permit ls --json handles errors
  {
    ...
    const result =
      await $`SKYNET_HOST=localhost:${port} SKYNET_HOME=${skynetHome} npx skynet permit ls --json`.nothrow();
  ...
  // Test 5: -q (quiet) flag is accepted globally
  {
    ...
    const result =
      await $`SKYNET_HOST=localhost:${port} SKYNET_HOME=${skynetHome} npx skynet -q permit ls`.nothrow();
    const output = result.stdout + result.stderr;
