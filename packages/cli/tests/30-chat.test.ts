...
try {
...

  {
    ...
    const posted = await ctx.skynet(["chat", "post", "coord-room", "first message for @agent-1"], {
      env: { SKYNET_AGENT_ID: "00000000-0000-4000-8000-000000000111" },
    });
    ...
    const read = await ctx.skynet(["chat", "read", "coord-room", "--limit", "10"]);
    ...
    const readJson = await ctx.skynet(["chat", "read", "coord-room", "--limit", "10", "--json"]);
    ...
    const waitPromise = ctx.skynet(["chat", "wait", "coord-room", "--timeout", "5s"]);
    await new Promise((resolve) => setTimeout(resolve, 250));
    const secondPost = await ctx.skynet(["chat", "post", "coord-room", "second message"]);
...

  {
    ...
    const duplicate = await ctx.skynet(["chat", "create", "coord-room"]);
...

  {
    ...
    const deleted = await ctx.skynet(["chat", "delete", "coord-room"]);
  ...
  await ctx.stop();
  await rm(ctx.skynetHome, { recursive: true, force: true });
