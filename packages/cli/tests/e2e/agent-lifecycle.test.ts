 ...
 * 2. Create agent with `skynet run "say hello" --provider claude`
 * 3. List agents with `skynet ls`
 * 4. Wait for agent with `skynet wait <id>`
 * 5. Inspect agent with `skynet inspect <id>`
 * 6. Stop agent with `skynet stop <id>` and verify it remains inspectable
 * 7. Delete agent with `skynet delete <id>`
...
async function setup(): Promise<void> {
  ...
  try {
    ...
    console.log(`SKYNET_HOME: ${ctx.skynetHome}`);
...
async function test_agent_ls_empty(): Promise<void> {
  ...
  const result = await ctx.skynet(["ls", "--json"]);
...
async function test_agent_wait(agentId: string): Promise<void> {
  ...
  const result = await ctx.skynet(["wait", "--timeout", "120s", agentId], { timeout: 130000 });
...
async function test_agent_inspect(agentId: string): Promise<void> {
  ...
  const result = await ctx.skynet(["inspect", agentId]);
...
async function test_agent_logs(agentId: string): Promise<void> {
  ...
  const result = await ctx.skynet(["logs", "--tail", "20", agentId]);
...
async function test_agent_stop(agentId: string): Promise<void> {
  ...
  const result = await ctx.skynet(["stop", agentId]);
  ...
  const inspectResult = await ctx.skynet(["inspect", agentId]);
...
async function test_agent_delete(agentId: string): Promise<void> {
  ...
  const result = await ctx.skynet(["delete", agentId]);
