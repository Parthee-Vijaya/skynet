...
const skynetHome = await mkdtemp(join(tmpdir(), "skynet-delete-test-home-"));
...
async function runDelete(args: string[]) {
  return $`SKYNET_HOST=localhost:${port} SKYNET_HOME=${skynetHome} npm --prefix ${repoRoot} run cli -- delete ${args}`.nothrow();
}
...
try {
  ...
  {
    console.log("Test 7: skynet --help shows delete command");
    ...
    console.log("✓ skynet --help shows delete command\n");
  }

  {
    console.log("Test 8: -q (quiet) flag is accepted with delete");
    const result =
      await $`SKYNET_HOST=localhost:${port} SKYNET_HOME=${skynetHome} npm --prefix ${repoRoot} run cli -- -q delete abc123`.nothrow();
    const output = result.stdout + result.stderr;
