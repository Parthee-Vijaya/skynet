...
const skynetHome = await mkdtemp(join(tmpdir(), "skynet-restart-supervisor-"));
...
try {
  ...
  const statusBeforeRestart = await readDaemonStatus(skynetHome);
  ...
  const statusAfterRestart = await readDaemonStatus(skynetHome);
  assert.strictEqual(
