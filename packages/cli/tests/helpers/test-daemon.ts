 ...
 * Test Daemon Helper
 *
 * Provides utilities for launching real Skynet daemons in E2E tests.
 * Each test gets an isolated daemon on an available local port with its own SKYNET_HOME.
 *
...
const TEST_OUTPUT_CAPTURE_LIMIT = Number.parseInt(
  process.env.SKYNET_TEST_OUTPUT_CAPTURE_BYTES ?? `${DEFAULT_OUTPUT_CAPTURE_LIMIT}`,
  10,
);
...
export async function createTempDirs(): Promise<{ skynetHome: string; workDir: string }> {
  const skynetHome = await mkdtemp(join(tmpdir(), "skynet-e2e-home-"));
  const workDir = await mkdtemp(join(tmpdir(), "skynet-e2e-work-"));

  // Create the agents directory that the daemon expects
  const agentsDir = join(skynetHome, "agents");
...
/**
 * Wait for daemon to be ready by running `skynet agent ls`
...
async function waitForDaemonReady(port: number, timeout = 30000): Promise<void> {
  ...
  while (Date.now() - start < timeout) {
    try {
      const { exitCode } = await runSkynetCli(
        {
          port,
 ...
 * with isolated SKYNET_HOME and SKYNET_LISTEN environment variables.
 */
export async function startTestDaemon(options?: {
  ...
  const { skynetHome, workDir } =
    options?.skynetHome && options?.workDir
      ? { skynetHome: options.skynetHome, workDir: options.workDir }
  ...
  const daemonProcess = spawn("npx", ["tsx", cliSrcPath, "daemon", "start", "--foreground"], {
    env: {
      ...
      SKYNET_LISTEN: `${TEST_DAEMON_HOST}:${port}`,
  ...
  return new Promise((resolve, reject) => {
    const proc = spawn("npx", ["tsx", cliSrcPath, ...args], {
      env: {
        ...process.env,
        ...TEST_DAEMON_ENV_DEFAULTS,
        SKYNET_HOST: `${TEST_DAEMON_HOST}:${ctx.port}`,
        SKYNET_HOME: ctx.skynetHome,
        ...options?.env,
      },
      cwd,
      stdio: ["ignore", "pipe", "pipe"],
    ...
    const timeoutId = setTimeout(() => {
      ...
      reject(new Error(`CLI command timed out after ${timeout}ms: skynet ${args.join(" ")}`));
    }, timeout);
...
export async function createE2ETestContext(options?: { timeout?: number }): Promise<
  TestDaemonContext & {
    /** Run a skynet CLI command against this daemon */
    skynet: (
      args: string[],
